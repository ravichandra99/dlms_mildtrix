...
class MILDDLMSActivityCalendar(MILDDLMSObject, IMILDDLMSBase):
    """
    Online help:
    http://www.mildtrix.fi/Mildtrix.DLMS.Objects.MILDDLMSActivityCalendar
    """
    ...
    def __init__(self, ln="0.0.13.0.0.255", sn=0):
        ...
        """
        MILDDLMSObject.__init__(self, ObjectType.ACTIVITY_CALENDAR, ln, sn)
    ...
    def __getSeasonProfile(cls, settings, target):
        ...
        if target is None:
            #  Add count
            _MILDCommon.setObjectCount(0, data)
        else:
            #  Add count
            _MILDCommon.setObjectCount(len(target), data)
            for it in target:
                data.setUInt8(DataType.STRUCTURE)
                data.setUInt8(3)
                _MILDCommon.setData(settings, data, DataType.OCTET_STRING, it.name)
                _MILDCommon.setData(settings, data, DataType.OCTET_STRING, it.start)
                _MILDCommon.setData(settings, data, DataType.OCTET_STRING, it.weekName)
        return data.array()
    ...
    def __getWeekProfileTable(cls, settings, target):
        ...
        if target is None:
            ...
            for it in target:
                ...
                data.setUInt8(8)
                _MILDCommon.setData(settings, data, DataType.OCTET_STRING, it.name)
                _MILDCommon.setData(settings, data, DataType.UINT8, it.monday)
                _MILDCommon.setData(settings, data, DataType.UINT8, it.tuesday)
                _MILDCommon.setData(settings, data, DataType.UINT8, it.wednesday)
                _MILDCommon.setData(settings, data, DataType.UINT8, it.thursday)
                _MILDCommon.setData(settings, data, DataType.UINT8, it.friday)
                _MILDCommon.setData(settings, data, DataType.UINT8, it.saturday)
                _MILDCommon.setData(settings, data, DataType.UINT8, it.sunday)
        return data.array()
    ...
    def __getDayProfileTable(cls, settings, target):
        ...
        if target is None:
            ...
            for it in target:
                data.setUInt8(DataType.STRUCTURE)
                data.setUInt8(2)
                _MILDCommon.setData(settings, data, DataType.UINT8, it.dayId)
                data.setUInt8(DataType.ARRAY)
                #  Add count
                _MILDCommon.setObjectCount(len(it.daySchedules), data)
                for action in it.daySchedules:
                    data.setUInt8(DataType.STRUCTURE)
                    data.setUInt8(3)
                    _MILDCommon.setData(settings, data, DataType.OCTET_STRING, action.startTime)
                    _MILDCommon.setData(settings, data, DataType.OCTET_STRING, _MILDCommon.logicalNameToBytes(action.scriptLogicalName))
                    _MILDCommon.setData(settings, data, DataType.UINT16, int(action.scriptSelector))
        return data.array()
    ...
    def getValue(self, settings, e):
        if e.index == 1:
            ...
            if not self.calendarNameActive:
                ...
                ret = MILDByteBuffer.hexToBytes(self.calendarNameActive)
            else:
            ...
            if self.calendarNamePassive is None:
                ...
                ret = MILDByteBuffer.hexToBytes(self.calendarNamePassive)
            else:
    ...
    def __setSeasonProfile(cls, settings, value):
        ...
        if value:
            for item in value:
                it = MILDDLMSSeasonProfile()
                it.name = item[0]
                it.start = _MILDCommon.changeType(settings, item[1], DataType.DATETIME)
                it.weekName = item[2]
                items.append(it)
        return items
    ...
    def __setDayProfileTable(cls, settings, value):
        ...
        if value:
            for item in value:
                it = MILDDLMSDayProfile()
                it.dayId = item[0]
                it.daySchedules = list()
                for it2 in item[1]:
                    ac = MILDDLMSDayProfileAction()
                    ac.startTime = _MILDCommon.changeType(settings, it2[0], DataType.TIME)
                    ac.scriptLogicalName = _MILDCommon.toLogicalName(it2[1])
    ...
    def setValue(self, settings, e):
        if e.index == 1:
            ...
            if self.isSec or not MILDByteBuffer.isAsciiString(e.value):
                self.calendarNameActive = MILDByteBuffer.hex(e.value)
            else:
            ...
            if self.isSec or not MILDByteBuffer.isAsciiString(e.value):
                self.calendarNamePassive = MILDByteBuffer.hex(e.value)
            else:
            ...
            if isinstance(e.value, MILDDateTime):
                self.time = e.value
            else:
                self.time = _MILDCommon.changeType(settings, e.value, DataType.DATETIME)
        else:
            e.error = ErrorCode.READ_WRITE_DENIED
    ...
    def loadSeasonProfile(cls, reader, name):
        ...
        if reader.isStartElement(name, True):
            while reader.isStartElement("Item", True):
                it = MILDDLMSSeasonProfile()
                it.name = MILDByteBuffer.hexToBytes(reader.readElementContentAsString("Name"))
                it.start = reader.readElementContentAsDateTime("Start")
                it.weekName = MILDByteBuffer.hexToBytes(reader.readElementContentAsString("WeekName"))
                list_.append(it)
        ...
        if list_:
            ...
            for it in list_:
                ...
                if isinstance(str_, (bytearray, bytes)):
                    str_ = MILDByteBuffer.hex(str_)
                writer.writeElementString("Name", str_)
                writer.writeElementString("Start", it.start)
                writer.writeElementString("WeekName", MILDByteBuffer.hex(it.weekName))
    ...
    def saveWeekProfileTable(cls, writer, list_, name):
        if list_:
            ...
            for it in list_:
                writer.writeStartElement("Item")
                writer.writeElementString("Name", MILDByteBuffer.hex(it.name))
