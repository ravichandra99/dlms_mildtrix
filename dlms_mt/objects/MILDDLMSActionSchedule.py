...
class MILDDLMSActionSchedule(MILDDLMSObject, IMILDDLMSBase):
    """
    Online help:
    http://www.mildtrix.fi/Mildtrix.DLMS.Objects.MILDDLMSActionSchedule
    """
    ...
    def __init__(self, ln="0.0.15.0.0.255", sn=0):
        ...
        """
        MILDDLMSObject.__init__(self, ObjectType.ACTION_SCHEDULE, ln, sn)
    ...
    def getValue(self, settings, e):
        ...
        if e.index == 2:
            ...
            bb.setUInt8(2)
            _MILDCommon.setData(settings, bb, DataType.OCTET_STRING, _MILDCommon.logicalNameToBytes(self.target.logicalName))
            _MILDCommon.setData(settings, bb, DataType.UINT16, int(self.executedScriptSelector))
            return bb.array()
        if e.index == 3:
            return self.type_
        if e.index == 4:
            ...
            if not self.executionTime:
                ...
                _MILDCommon.setObjectCount(len(self.executionTime), bb)
                for it in self.executionTime:
                    bb.setUInt8(DataType.STRUCTURE)
                    #  Count
                    bb.setUInt8(2)
                    #  Time
                    _MILDCommon.setData(settings, bb, DataType.OCTET_STRING, MILDTime(it))
                    #  Date
                    _MILDCommon.setData(settings, bb, DataType.OCTET_STRING, MILDDate(it))
            return bb.array()
    ...
    def setValue(self, settings, e):
        if e.index == 1:
            ...
            if e.value:
                ln = _MILDCommon.toLogicalName(e.value[0])
                self.target = settings.objects.findByLN(ObjectType.SCRIPT_TABLE, ln)
                if not self.target:
                    self.target = MILDDLMSScriptTable(ln)
            ...
            if e.value:
                for it in e.value:
                    time = MILDDateTime(_MILDCommon.changeType(settings, it[0], DataType.TIME))
                    date = MILDDateTime(_MILDCommon.changeType(settings, it[1], DataType.DATE))
                    tmp = MILDDateTime(date)
