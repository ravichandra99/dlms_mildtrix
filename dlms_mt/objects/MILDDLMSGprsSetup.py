...
class MILDDLMSGprsSetup(MILDDLMSObject, IMILDDLMSBase):
    """
    Online help:
    http://www.mildtrix.fi/Mildtrix.DLMS.Objects.MILDDLMSGprsSetup
    """
    ...
    def __init__(self, ln="0.0.25.4.0.255", sn=0):
        ...
        """
        MILDDLMSObject.__init__(self, ObjectType.GPRS_SETUP, ln, sn)
        self.apn = ""
        self.pinCode = 0
        self.defaultQualityOfService = MILDDLMSQualityOfService()
        self.requestedQualityOfService = MILDDLMSQualityOfService()
    ...
    def getValue(self, settings, e):
        ...
        if e.index == 1:
            ...
            data.setUInt8(DataType.STRUCTURE)
            data.setUInt8(5)
            _MILDCommon.setData(settings, data, DataType.UINT8, self.defaultQualityOfService.precedence)
            _MILDCommon.setData(settings, data, DataType.UINT8, self.defaultQualityOfService.delay)
            _MILDCommon.setData(settings, data, DataType.UINT8, self.defaultQualityOfService.reliability)
            _MILDCommon.setData(settings, data, DataType.UINT8, self.defaultQualityOfService.peakThroughput)
            _MILDCommon.setData(settings, data, DataType.UINT8, self.defaultQualityOfService.meanThroughput)
            data.setUInt8(DataType.STRUCTURE)
            data.setUInt8(5)
            _MILDCommon.setData(settings, data, DataType.UINT8, self.requestedQualityOfService.precedence)
            _MILDCommon.setData(settings, data, DataType.UINT8, self.requestedQualityOfService.delay)
            _MILDCommon.setData(settings, data, DataType.UINT8, self.requestedQualityOfService.reliability)
            _MILDCommon.setData(settings, data, DataType.UINT8, self.requestedQualityOfService.peakThroughput)
            _MILDCommon.setData(settings, data, DataType.UINT8, self.requestedQualityOfService.meanThroughput)
            ret = data.array()
        else:
    ...
    def setValue(self, settings, e):
        if e.index == 1:
            ...
            if isinstance(e.value, str):
                ...
                self.apn = _MILDCommon.changeType(settings, e.value, DataType.STRING)
        elif e.index == 3:
