...
class MILDDLMSPppSetup(MILDDLMSObject, IMILDDLMSBase):
    """
    Online help:
    http://www.mildtrix.fi/Mildtrix.DLMS.Objects.MILDDLMSPppSetup
    """
    ...
    def __init__(self, ln=None, sn=0):
        super(MILDDLMSPppSetup, self).__init__(ObjectType.PPP_SETUP, ln, sn)
    ...
    def getValue(self, settings, e):
        if e.index == 1:
            ...
            ret = _MILDCommon.logicalNameToBytes(self.phyReference)
        elif e.index == 3:
            data = MILDByteBuffer()
            ...
            if not self.lcpOptions:
                ...
                for it in self.lcpOptions:
                    ...
                    data.setUInt8(3)
                    _MILDCommon.setData(settings, data, DataType.UINT8, it.type_.value)
                    _MILDCommon.setData(settings, data, DataType.UINT8, it.length)
                    _MILDCommon.setData(settings, data, _MILDCommon.getDLMSDataType(it.data), it.data)
            ret = data.array()
        elif e.index == 4:
            data = MILDByteBuffer()
            data.setUInt8(DataType.ARRAY)
            ...
            if self.userName:
                ...
                data.setUInt8(DataType.STRUCTURE)
                data.setUInt8(2)
                _MILDCommon.setData(settings, data, DataType.OCTET_STRING, self.userName)
                _MILDCommon.setData(settings, data, DataType.OCTET_STRING, self.password)
                ret = data.array()
            else:
                ret = None
        else:
    ...
    def setValue(self, settings, e):
        if e.index == 1:
            ...
            self.phyReference = _MILDCommon.toLogicalName(e.value)
        elif e.index == 3:
            self.lcpOptions = []
            if e.value:
                for item in e.value:
                    it1 = MILDDLMSPppSetupLcpOption()
            ...
            if e.value:
                for item in e.value:
                    it2 = MILDDLMSPppSetupIPCPOption()
                    it2.type_ = item[0]
                    it2.length = item[1]
                    it2.data = item[2]
    ...
    def load(self, reader):
        ...
        self.userName = MILDByteBuffer.hexToBytes(reader.readElementContentAsString("UserName"))
        self.password = MILDByteBuffer.hexToBytes(reader.readElementContentAsString("Password"))
    ...
    def save(self, writer):
        ...
        if self.ipcpOptions:
            for it in self.ipcpOptions:
                ...
                writer.writeEndElement()
        writer.writeEndElement()
        writer.writeElementString("UserName", MILDByteBuffer.hex(self.userName))
        writer.writeElementString("Password", MILDByteBuffer.hex(self.password))
