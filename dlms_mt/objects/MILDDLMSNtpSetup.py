...
class MILDDLMSNtpSetup(MILDDLMSObject, IMILDDLMSBase):
    """
    Online help:
    http://www.mildtrix.fi/Mildtrix.DLMS.Objects.MILDDLMSNtpSetup
    """
    ...
    def __init__(self, ln="0.0.25.10.0.255", sn=0):
        ...
        """
        MILDDLMSObject.__init__(self, ObjectType.NTP_SETUP, ln, sn)
        self.value = None
    ...
    def addAuthenticationKey(self, client, id_, key):
        ...
        _MILDCommon.setObjectCount(key.length, bb)
        bb.set(key)
    ...
    def getUIDataType(self, index):
        ...
        return super(MILDDLMSNtpSetup, self).getUIDataType(index)
    ...
    def getValue(self, settings, e):
        if e.index == 1:
            ...
            #Add count
            _MILDCommon.setObjectCount(len(self.keys), bb)
            for it in self.keys:
                bb.setUInt8(DataType.STRUCTURE)
                #Count
                bb.setUInt8(2)
                ...
                bb.setUInt32(it.key)
                bb.setUInt8(DataType.OCTET_STRING)
                _MILDCommon.setObjectCount(len(it.value), bb)
                bb.set(it.value)
            ret = bb.array()
        elif e.index == 7:
            ret = self.clientKey
    ...
    def load(self, reader):
        ...
        if reader.isStartElement("Keys", True):
            while reader.isStartElement("Item", True):
                ...
                key = MILDByteBuffer.hexToBytes(reader.readElementContentAsString("Key"))
                self.keys[id_] = key
        self.clientKey = MILDByteBuffer.hexToBytes(reader.readElementContentAsString("ServerAddress", None))
    ...
    def save(self, writer):
        ...
        for it in self.keys:
            ...
            writer.writeElementString("Key", MILDByteBuffer.toHex(it.value, False))
            writer.writeEndElement()
        writer.writeEndElement()
        #Keys
        writer.writeElementString("ClientKey", MILDByteBuffer.toHex(self.clientKey, False))
