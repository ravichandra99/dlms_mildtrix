...
class MILDDLMSAutoAnswer(MILDDLMSObject, IMILDDLMSBase):
    """
    Online help:
    http://www.mildtrix.fi/Mildtrix.DLMS.Objects.MILDDLMSAutoAnswer
    """
    ...
    def __init__(self, ln="0.0.2.2.0.255", sn=0):
        ...
        """
        MILDDLMSObject.__init__(self, ObjectType.AUTO_ANSWER, ln, sn)
    ...
    def getValue(self, settings, e):
        ...
        if e.index == 1:
            ...
            #  Add count
            _MILDCommon.setObjectCount(cnt, buff)
            if cnt != 0:
                for it in self.listeningWindow:
                    ...
                    buff.setUInt8(2)
                    #  Start time
                    _MILDCommon.setData(settings, buff, DataType.OCTET_STRING, it[0])
                    #  End time
                    _MILDCommon.setData(settings, buff, DataType.OCTET_STRING, it[1])
            ret = buff.array()
        elif e.index == 4:
            ret = self.status
        elif e.index == 5:
            ...
            buff = MILDByteBuffer()
            buff.setUInt8(DataType.STRUCTURE)
            _MILDCommon.setObjectCount(2, buff)
            _MILDCommon.setData(settings, buff, DataType.UINT8, self.numberOfRingsInListeningWindow)
            _MILDCommon.setData(settings, buff, DataType.UINT8, self.numberOfRingsOutListeningWindow)
            ret = buff.array()
        else:
