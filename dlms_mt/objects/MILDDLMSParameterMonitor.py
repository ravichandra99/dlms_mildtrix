...
class MILDDLMSParameterMonitor(MILDDLMSObject, IMILDDLMSBase):
    """
    Online help:
    http://www.mildtrix.fi/Mildtrix.DLMS.Objects.MILDDLMSParameterMonitor
    """
    def __init__(self, ln="0.0.16.2.0.255", sn=0):
        ...
        """
        super(MILDDLMSParameterMonitor, self).__init__(ObjectType.PARAMETER_MONITOR, ln, sn)
        self.parameters = list()
        self.captureTime = None
        self.changedParameter = MILDDLMSTarget()
    ...
    def insert(self, client, entry):
        ...
        bb.setUInt8(3)
        _MILDCommon.setData(None, bb, DataType.UINT16, entry.target.objectType)
        _MILDCommon.setData(None, bb, DataType.OCTET_STRING, _MILDCommon.logicalNameToBytes(entry.target.logicalName))
        _MILDCommon.setData(None, bb, DataType.INT8, entry.attributeIndex)
        ...
        if e.index != 1 and e.index != 2:
            ...
            if e.index == 1:
                ...
                if it.target is None:
                    it.target = _MILDObjectFactory.createObject(type_)
    ...
    def getValue(self, settings, e):
        ...
        if e.index == 1:
            ...
            if self.changedParameter is None:
                ...
                _MILDCommon.setData(settings, data, DataType.OCTET_STRING, [0, 0, 0, 0, 0, 0])
                _MILDCommon.setData(settings, data, DataType.INT8, 1)
                _MILDCommon.setData(settings, data, DataType.NONE, None)
            else:
                _MILDCommon.setData(settings, data, DataType.UINT16, self.changedParameter.target.objectType)
                _MILDCommon.setData(settings, data, DataType.OCTET_STRING, _MILDCommon.logicalNameToBytes(self.changedParameter.target.logicalName))
                _MILDCommon.setData(settings, data, DataType.INT8, self.changedParameter.attributeIndex)
                _MILDCommon.setData(settings, data, _MILDCommon.getDLMSDataType(self.changedParameter.value), self.changedParameter.value)
            ret = data
        elif e.index == 3:
            ...
            if self.parameters is None:
                ...
                for it in self.parameters:
                    data.setUInt8(DataType.STRUCTURE)
                    data.setUInt8(3)
                    _MILDCommon.setData(settings, data, DataType.UINT16, it.target.objectType)
                    _MILDCommon.setData(settings, data, DataType.OCTET_STRING, _MILDCommon.logicalNameToBytes(it.target.logicalName))
                    _MILDCommon.setData(settings, data, DataType.INT8, it.attributeIndex)
            ret = data
        else:
            e.error = ErrorCode.READ_WRITE_DENIED
        return ret
    ...
    def setValue(self, settings, e):
        ...
        if e.index == 1:
            ...
            if self.changedParameter.target is None:
                self.changedParameter.target = _MILDObjectFactory.createObject(type_)
            ...
            if e.value is None:
                ...
                if isinstance(e.value, bytearray):
                    self.captureTime = _MILDCommon.changeType(settings, e.value, DataType.DATETIME)
                else:
            ...
            if e.value:
                for i in e.value:
                    ...
                    ln = _MILDCommon.toLogicalName(i[1])
                    obj.target = settings.objects.findByLN(type_, ln)
                    if obj.target is None:
                        obj.target = _MILDObjectFactory.createObject(type_)
    ...
    def load(self, reader):
        ...
        if reader.isStartElement("ChangedParameter", True):
            ...
            if self.changedParameter.target is None:
                self.changedParameter.target = _MILDObjectFactory.createObject(ot)
                self.changedParameter.target.logicalName = ln
        ...
        if reader.isStartElement("Parameters", True):
            while reader.isStartElement("Item", True):
                ...
                if obj.target is None:
                    obj.target = _MILDObjectFactory.createObject(ot)
                    obj.target.logicalName = ln
