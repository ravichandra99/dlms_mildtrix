...
class MILDDLMSClock(MILDDLMSObject, IMILDDLMSBase):
    """
    Online help:
    http://www.mildtrix.fi/Mildtrix.DLMS.Objects.MILDDLMSClock
    """
    ...
    def __init__(self, ln="0.0.1.0.0.255", sn=0):
        ...
        """
        MILDDLMSObject.__init__(self, ObjectType.CLOCK, ln, sn)
        self.time = None
        self.timeZone = 0
        self.status = ClockStatus.OK
        self.deviation = 0
        self.begin = MILDDateTime()
        self.end = MILDDateTime()
        self.enabled = False
    ...
    def getUIDataType(self, index):
        ...
        return super(MILDDLMSClock, self).getUIDataType(index)
    ...
    def presetAdjustingTime(self, client, presetTime, validityIntervalStart, validityIntervalEnd):
        buff = MILDByteBuffer(44)
        buff.setUInt8(DataType.STRUCTURE)
        buff.setUInt8(3)
        _MILDCommon.setData(None, buff, DataType.OCTET_STRING, presetTime)
        _MILDCommon.setData(None, buff, DataType.OCTET_STRING, validityIntervalStart)
        _MILDCommon.setData(None, buff, DataType.OCTET_STRING, validityIntervalEnd)
    ...
    def setValue(self, settings, e):
        ...
        if e.index == 1:
            ...
            if e.value is None:
                ...
                self.begin = _MILDCommon.changeType(settings, e.value, DataType.DATETIME)
            else:
                self.begin = e.value
        elif e.index == 6:
            if e.value is None:
                ...
                self.end = _MILDCommon.changeType(settings, e.value, DataType.DATETIME)
            else:
                self.end = e.value
        elif e.index == 7:
            if e.value is None:
