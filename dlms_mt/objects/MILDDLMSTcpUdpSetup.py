...
class MILDDLMSTcpUdpSetup(MILDDLMSObject, IMILDDLMSBase):
    """
    Online help:
    http://www.mildtrix.fi/Mildtrix.DLMS.Objects.MILDDLMSTcpUdpSetup
    """
    def __init__(self, ln="0.0.25.0.0.255", sn=0):
        ...
        """
        super(MILDDLMSTcpUdpSetup, self).__init__(ObjectType.TCP_UDP_SETUP, ln, sn)
        self.port = 4059
    ...
    def getValue(self, settings, e):
        if e.index == 1:
            ...
            ret = _MILDCommon.logicalNameToBytes(self.ipReference)
        elif e.index == 4:
        ...
        if e.index == 1:
            ...
            if e.value is None:
                ...
                if isinstance(e.value, bytearray):
                    self.ipReference = _MILDCommon.toLogicalName(e.value)
                else:
