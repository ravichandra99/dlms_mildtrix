...
class MILDDLMSSpecialDaysTable(MILDDLMSObject, IMILDDLMSBase):
    """
    Online help:
    http://www.mildtrix.fi/Mildtrix.DLMS.Objects.MILDDLMSSpecialDaysTable
    """
    ...
    def __init__(self, ln="0.0.11.0.0.255", sn=0):
        ...
        """
        super(MILDDLMSSpecialDaysTable, self).__init__(ObjectType.SPECIAL_DAYS_TABLE, ln, sn)
        # Special day entries.
        self.entries = list()
    ...
    def insert(self, client, entry):
        ...
        bb.setUInt8(3)
        _MILDCommon.setData(None, bb, DataType.UINT16, entry.index)
        _MILDCommon.setData(None, bb, DataType.OCTET_STRING, entry.date)
        _MILDCommon.setData(None, bb, DataType.UINT8, entry.dayId)
        ...
        if e.index == 2:
            ...
            if not self.entries:
                ...
                for it in self.entries:
                    ...
                    data.setUInt8(3)
                    #  Count
                    _MILDCommon.setData(settings, data, DataType.UINT16, it.index)
                    _MILDCommon.setData(settings, data, DataType.OCTET_STRING, it.date)
    ...
    def setValue(self, settings, e):
        if e.index == 1:
            ...
            if e.value:
                for item in e.value:
                    it = MILDDLMSSpecialDay()
                    it.index = item[0]
                    it.date = _MILDCommon.changeType(settings, item[1], DataType.DATE)
                    it.dayId = item[2]
