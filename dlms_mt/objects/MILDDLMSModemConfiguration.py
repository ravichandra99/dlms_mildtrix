...
class MILDDLMSModemConfiguration(MILDDLMSObject, IMILDDLMSBase):
    """
    Online help:
    http://www.mildtrix.fi/Mildtrix.DLMS.Objects.MILDDLMSModemConfiguration
    """
    ...
    def __init__(self, ln=None, sn=0):
        super(MILDDLMSModemConfiguration, self).__init__(ObjectType.MODEM_CONFIGURATION, ln, sn)
        ...
        if e.index == 3:
            ...
            if cnt != 0:
                for it in self.initialisationStrings:
                    ...
                    data.setUInt8(3)
                    #  Count
                    _MILDCommon.setData(settings, data, DataType.OCTET_STRING, _MILDCommon.getBytes(it.request))
                    _MILDCommon.setData(settings, data, DataType.OCTET_STRING, _MILDCommon.getBytes(it.response))
                    _MILDCommon.setData(settings, data, DataType.UINT16, it.delay)
            return data
        if e.index == 4:
            data = MILDByteBuffer()
            data.setUInt8(DataType.ARRAY)
    ...
    def setValue(self, settings, e):
        if e.index == 1:
            ...
            if e.value:
                for it in e.value:
                    item = MILDDLMSModemInitialisation()
    ...
    def load(self, reader):
        ...
        if reader.isStartElement("InitialisationStrings", True):
            while reader.isStartElement("Initialisation", True):
                it = MILDDLMSModemInitialisation()
