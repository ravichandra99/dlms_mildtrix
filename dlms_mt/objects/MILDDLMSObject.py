...
from ..manufacturersettings import MILDAttributeCollection, MILDDLMSAttributeSettings
...
class MILDDLMSObject(object):
    ...
    def __init__(self, ot, ln=None, sn=0):
        ...
        self.description = ""
        self.attributes = MILDAttributeCollection()
        self.methodAttributes = MILDAttributeCollection()
        if ln:
            items = ln.split('.')
            if len(items) != 6:
    ...
    def validateLogicalName(cls, value):
        _MILDCommon.logicalNameToBytes(value)
    ...
    def setAccess(self, index, access):
        att = self.attributes.find(index)
        if att is None:
            att = MILDDLMSAttributeSettings(index)
