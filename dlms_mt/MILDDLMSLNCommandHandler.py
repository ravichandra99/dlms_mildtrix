...
class MILDDLMSLNCommandHandler:
    #Constructor.
    def __init__(self):
        pass
    ...
    def handleGetRequest(cls, settings, server, data, replyData, xml):
        ...
        if type_ == GetCommandType.NORMAL:
            ...
            MILDDLMS.getLNPdu(MILDDLMSLNParameters(settings, invokeID, Command.GET_RESPONSE, type_, None, bb, ErrorCode.OK), replyData)
        if xml:
    ...
    def handleSetRequest(cls, settings, server, data, replyData, xml):
        ...
        p = MILDDLMSLNParameters(settings, invoke, Command.SET_RESPONSE, type_, None, None, 0)
        if xml:
    ...
    def appendAttributeDescriptor(cls, xml, ci, ln, attributeIndex):
        ...
        xml.appendComment(_MILDCommon.toLogicalName(ln))
        xml.appendLine(TranslatorTags.INSTANCE_ID, "Value", MILDByteBuffer.hex(ln, False))
    ...
    def getRequestNormal(cls, settings, invokeID, server, data, replyData, xml):
        ...
        if xml:
            ...
            if selection != 0:
                ...
                _MILDCommon.getData(settings, data, info)
                xml.appendEndTag(TranslatorTags.ACCESS_PARAMETERS)
                xml.appendEndTag(TranslatorTags.ACCESS_SELECTION)
            return
        if selection != 0:
            parameters = _MILDCommon.getData(settings, data, info)
        ot = ObjectType(ci)
        obj = settings.objects.findByLN(ot, _MILDCommon.toLogicalName(ln))
        if obj is None:
            obj = server.onFindObject(ot, 0, _MILDCommon.toLogicalName(ln))
        ...
        if obj is None:
            ...
            if server.onGetAttributeAccess(e) == AccessMode.NO_ACCESS:
                ...
                if isinstance(obj, (MILDDLMSProfileGeneric,)) and attributeIndex == 2:
                    dt = None
                    rowsize = 0
                    pg = e.target
                    ...
                    for k, v in pg.captureObjects:
                        ...
                        if dt == DataType.OCTET_STRING:
                            ...
                            if dt == DataType.DATETIME:
                                rowsize += _MILDCommon.getDataTypeSize(DataType.DATETIME)
                            elif dt == DataType.DATE:
                                rowsize += _MILDCommon.getDataTypeSize(DataType.DATE)
                            elif dt == DataType.TIME:
                                rowsize += _MILDCommon.getDataTypeSize(DataType.TIME)
                        elif dt == DataType.NONE:
                            rowsize += 2
                        else:
                            rowsize += _MILDCommon.getDataTypeSize(dt)
                    if rowsize != 0:
                ...
                if e.byteArray:
                    ...
                    MILDDLMS.appendData(settings, obj, attributeIndex, bb, value)
                status = e.error
        MILDDLMS.getLNPdu(MILDDLMSLNParameters(settings, e.invokeId, Command.GET_RESPONSE, 1, None, bb, status), replyData)
        if settings.count != settings.index or len(bb) != bb.position:
            server.setTransaction(MILDDLMSLongTransaction(e, Command.GET_REQUEST, bb))
    ...
    def getRequestNextDataBlock(cls, settings, invokeID, server, data, replyData, xml, streaming):
        ...
        if not streaming:
            ...
            if index != settings.blockIndex:
                MILDDLMS.getLNPdu(MILDDLMSLNParameters(settings, invokeID, Command.GET_RESPONSE, 2, None, bb, ErrorCode.DATA_BLOCK_NUMBER_INVALID), replyData)
                return
        settings.increaseBlockIndex()
        p = MILDDLMSLNParameters(settings, invokeID, Command.GENERAL_BLOCK_TRANSFER if streaming else Command.GET_RESPONSE, 2, None, bb, ErrorCode.OK)
                ...
                if len(bb) < settings.getMaxPduSize():
                    ...
                    for arg in server.transaction.targets:
                        ...
                        if arg.byteArray:
                            ...
                            MILDDLMS.appendData(settings, arg.target, arg.index, bb, value)
                    moreData = settings.index != settings.getCount()
    ...
    def getRequestWithList(cls, settings, invokeID, server, data, replyData, xml):
        ...
        while pos != cnt:
            ...
            if selection != 0:
                selector = data.getUInt8()
                i = _MILDDataInfo()
                parameters = _MILDCommon.getData(settings, data, i)
            if xml:
                ...
                obj = settings.objects.findByLN(ci, _MILDCommon.toLogicalName(ln))
                if obj is None:
                    obj = server.onFindObject(ci, 0, _MILDCommon.toLogicalName(ln))
        ...
        value = None
        pos = 0
        p = MILDDLMSLNParameters(settings, invokeID, Command.GET_RESPONSE, 3, None, bb, 0xFF)
        for it in list_:
            try:
                if it.handled:
                    value = it.value
                ...
                if it.byteArray:
                    ...
                    MILDDLMS.appendData(settings, it.target, it.index, bb, value)
                p.invokeId = it.invokeId
            except Exception:
                bb.setUInt8(ErrorCode.HARDWARE_FAULT)
            if settings.index != settings.count:
                server.setTransaction(MILDDLMSLongTransaction(list_, Command.GET_REQUEST, None))
            pos += 1
    ...
    def handleSetRequestNormal(cls, settings, server, data, type_, p, xml):
        ...
        if type_ == 2:
            ...
            if blockNumber != settings.blockIndex:
                ...
                return
            settings.increaseBlockIndex()
            size = _MILDCommon.getObjectCount(data)
        ...
        if xml:
            ...
            di = _MILDDataInfo()
            di.xml = (xml)
            value = _MILDCommon.getData(settings, data, di)
            if not di.complete:
                value = MILDByteBuffer.hex(data.data, False, data.position, len(data) - data.position)
            elif isinstance(value, bytearray):
                value = MILDByteBuffer.hex(value, False)
            xml.appendEndTag(TranslatorTags.VALUE)
            return
        if not p.isMultipleBlocks():
            settings.resetBlockIndex()
            value = _MILDCommon.getData(settings, data, reply)
        obj = settings.objects.findByLN(ot, _MILDCommon.toLogicalName(ln))
        if obj is None:
            obj = server.onFindObject(ot, 0, _MILDCommon.toLogicalName(ln))
                ...
                try:
                    if isinstance(value, bytearray):
                        ...
                        if dt not in (DataType.NONE, DataType.OCTET_STRING):
                            value = _MILDCommon.changeType(settings, value, dt)
                    e.value = value
                    list_ = list(e)
                    if p.isMultipleBlocks():
                        server.setTransaction(MILDDLMSLongTransaction(list_, Command.GET_REQUEST, data))
    ...
    def hanleSetRequestWithDataBlock(cls, settings, server, data, p, xml):
        ...
        if xml is None and blockNumber != settings.blockIndex:
            ...
            if not p.isMultipleBlocks():
                try:
                    value = _MILDCommon.getData(settings, server.getTransaction().data, reply)
                    if isinstance(value, bytearray):
    ...
    def handleMethodRequest(cls, settings, server, data, connectionInfo, replyData, xml):
        ...
        if xml:
            ...
            if type_ == ActionRequestType.NORMAL:
                ...
                if selection != 0:
                    ...
                    di = _MILDDataInfo()
                    di.xml = (xml)
                    _MILDCommon.getData(settings, data, di)
            ...
            if server.onGetMethodAccess(e) == MethodAccessMode.NO_ACCESS:
                ...
                if actionReply and e.error == ErrorCode.OK:
                    ...
                    if e.byteArray:
                        ...
                        _MILDCommon.setData(settings, bb, _MILDCommon.getDLMSDataType(actionReply), actionReply)
                else:
                    error = e.error
                    bb.setUInt8(0)
                invokeId = int(e.invokeId)
        p = MILDDLMSLNParameters(settings, invokeId, Command.METHOD_RESPONSE, 1, None, bb, error)
        MILDDLMS.getLNPdu(p, replyData)
        if isinstance(obj, (MILDDLMSAssociationLogicalName,)) and id_ == 1:
        ...
        if e is not None and error == 0 and isinstance(obj, MILDDLMSSecuritySetup) and id_ == 2:
    ...
    def handleAccessRequest(cls, settings, server, data, reply, xml):
        ...
        if len_ != 0:
            ...
            if xml is None:
                ...
                info = _MILDDataInfo()
                info.type = dt
                _MILDCommon.getData(settings, MILDByteBuffer(tmp), info)
        cnt = _MILDCommon.getObjectCount(data)
        if xml:
            xml.appendStartTag(Command.ACCESS_REQUEST)
            xml.appendLine(TranslatorTags.LONG_INVOKE_ID, "Value", xml.integerToHex(invokeId, 8))
            xml.appendLine(TranslatorTags.DATE_TIME, "Value", MILDByteBuffer.hex(tmp, False))
    ...
    def handleEventNotification(cls, settings, reply, list_):
        ...
        if reply.xml:
            ...
            if reply.time:
                ...
                reply.xml.appendLine(TranslatorTags.TIME, None, MILDByteBuffer.hex(tmp, False))
        ci = data.getUInt16()
        ln = bytearray(6)
        data.get(ln)
        index = data.getUInt8()
        ...
        di = _MILDDataInfo()
        di.xml = reply.xml
        value = _MILDCommon.getData(settings, reply.data, di)
        if reply.xml:
            reply.xml.appendEndTag(TranslatorTags.ATTRIBUTE_VALUE)
            reply.xml.appendEndTag(Command.EVENT_NOTIFICATION)
        else:
            obj = settings.objects.findByLN(ObjectType(ci), _MILDCommon.toLogicalName(ln))
            if obj:
                ...
                print("InformationReport message. Unknown object : " + str(ObjectType(ci)) + " " + _MILDCommon.toLogicalName(ln))
