...
class MILDByteBuffer(__base):
    ...
    def set(self, value, index=None, count=None):
        ...
        if value:
            if index is None:
                if isinstance(value, MILDByteBuffer):
    ...
    def ___getValue(cls, c):
        ...
        if c.islower():
            c = c.upper()
        pos = MILDByteBuffer.__HEX_ARRAY.find(c)
        if pos == -1:
    ...
    def hexToBytes(cls, value):
        ...
        if value:
            ...
            for ch in value:
                if ch != ' ':
                    if lastValue == -1:
                        lastValue = MILDByteBuffer.___getValue(ch)
                    elif lastValue != -1:
                        buff.append(lastValue << MILDByteBuffer.__NIBBLE | MILDByteBuffer.___getValue(ch))
                        lastValue = -1
                elif lastValue != -1:
                    buff.append(MILDByteBuffer.___getValue(ch))
                    lastValue = -1
        return buff
    ...
    def hex(cls, value, addSpace=True, index=0, count=None):
        ...
        for it in value[index:count]:
            hexChars += MILDByteBuffer.__HEX_ARRAY[it >> MILDByteBuffer.__NIBBLE]
            hexChars += MILDByteBuffer.__HEX_ARRAY[it & MILDByteBuffer.__LOW_BYTE_PART]
            if addSpace:
                hexChars += ' '
