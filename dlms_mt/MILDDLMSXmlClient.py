...
class MILDDLMSXmlClient(MILDDLMSSecureClient):
    """
    MILDDLMS Xml client implements methods to communicate with DLMS/COSEM metering
    devices using XML.
    """
    ...
    def __init__(self, type_):
        MILDDLMSSecureClient.__init__(self)
        ...
        self.translator = MILDDLMSTranslator(type_)
            ...
            if m1.getNodeType() == Node.ELEMENT_NODE:
                ...
                while pos2 != m1.getChildNodes().getLength():
                    ...
                    if node.getNodeType() == Node.ELEMENT_NODE:
                            ...
                            if loadSettings.start() and loadSettings.end:
                                ...
                                while pos3 != node.getChildNodes().getLength():
                                    ...
                                    if n1.getNodeName() == "GetRequestNormal":
                                                            ...
                                                            if n4.getNodeName() == "Structure":
                                                                ...
                                                                while pos7 != n4.getChildNodes().getLength():
                                                                    ...
                                                                    if n5.getNodeName() == "OctetString":
                                                                        bb = MILDByteBuffer()
                                                                        if start:
                                                                            _MILDCommon.setData(self.settings, bb, DataType.OCTET_STRING, loadSettings.getStart())
                                                                            n5.getAttributes().getNamedItem("Value").setNodeValue(bb.toHex(False, 2))
                                                                            start = False
                                                                        else:
                                                                            _MILDCommon.setData(self.settings, bb, DataType.OCTET_STRING, loadSettings.getEnd())
                                                                            n5.getAttributes().getNamedItem("Value").setNodeValue(bb.toHex(False, 2))
                                                                            break
                                                                    pos7 += 1
                                                                break
                                        ...
                                        break
                                    pos3 += 1
                        s = MILDDLMSXmlSettings(self.translator.outputType, self.translator.hex, self.translator.showStringAsHex, self.translator.tagsByName)
                        s.settings.clientAddress = self.settings.clientAddress
                        s.settings.serverAddress = self.settings.serverAddress
                        reply = []
                        reply = self.translator.xmlToPdu(MILDDLMSXmlPdu.getOuterXml(node), s)
                        ...
                        if s.template:
                            reply = None
                        p = MILDDLMSXmlPdu(s.command, node, reply)
                        if description:
    ...
    def pduToMessages(self, pdu):
        ...
        if pdu.command == Command.SNRM:
            ...
            messages.append(MILDDLMS.getHdlcFrame(self.settings, int(Command.DISCONNECT_REQUEST), MILDByteBuffer(pdu.data)))
        else:
            reply = None
            if self.settings.interfaceType == InterfaceType.WRAPPER:
                if self.ciphering.security != Security.NONE:
                    p = MILDDLMSLNParameters(self.settings, 0, pdu.command, 0x0, None, None, 0xff)
                    reply = MILDByteBuffer(MILDDLMS.cipher0(p, pdu.data))
                else:
                    reply = MILDByteBuffer(pdu.data)
            else:
                if self.ciphering.security != Security.NONE:
                    p = MILDDLMSLNParameters(self.settings, 0, pdu.command, 0x0, None, None, 0xff)
                    tmp = MILDDLMS.cipher0(p, pdu.data)
                    reply = MILDByteBuffer(len(tmp))
                    reply.set(_MILDCommon.LLC_SEND_BYTES)
                    reply.set(tmp)
                else:
                    reply = MILDByteBuffer(len(pdu.data))
                    reply.set(_MILDCommon.LLC_SEND_BYTES)
                    reply.set(pdu.data)
            frame_ = 0
            while reply.position != len(reply):
                if self.settings.interfaceType == InterfaceType.WRAPPER:
                    messages.append(MILDDLMS.getWrapperFrame(self.settings, pdu.command, reply))
                    ...
                    if pdu.command == Command.AARQ:
                        ...
                        frame_ = 0x13
                    messages.append(MILDDLMS.getHdlcFrame(self.settings, frame_, reply))
