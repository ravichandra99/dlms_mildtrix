...
# ---------------------------------------------------------------------------
from dlms_mt.enums import InterfaceType, Authentication, Security, Standard
from dlms_mt import MILDDLMSClient
from dlms_mt.secure import MILDDLMSSecureClient
from dlms_mt.MILDByteBuffer import MILDByteBuffer
from dlms_mt.objects import MILDDLMSObject
...
class MILDSettings:
    ...
    def __init__(self):
        ...
        #  Objects to read.
        self.readObjects = []
        self.outputFile = '/var/www/mildtrix_yash/Mildtrix.DLMS.Client.Example.python/device.xml'
    ...
    @classmethod
    def showHelp(cls):
        print("MildtrixDlmsSample reads data from the DLMS/COSEM device.")
        print("MildtrixDlmsSample -h [Meter IP Address] -p [Meter Port No] -c 16 -s 1 -r SN")
        ...
        print("MildtrixDlmsSample -r SN -c 16 -s 1 -h [Meter IP Address] -p [Meter Port No]")
        print("Read LG device using serial port connection.")
        print("MildtrixDlmsSample -r SN -c 16 -s 1 -sp COM1 -i")
        print("Read Indian device using serial port connection.")
        print("MildtrixDlmsSample -S COM1 -c 16 -s 1 -a Low -P [password]")
        print("------------------------------------------------------")
    ...
    def getParameters(self, args):
        parameters = MILDSettings.__getParameters(args, "h:p:c:s:r:i:It:a:p:P:g:S:n:C:v:o:T:A:B:D:d:l:W:w:f:L:")
        ...
        for it in parameters:
            if it.tag == 'r':
                ...
                if it.value.startswith("0x"):
                    self.client.password = MILDByteBuffer.hexToBytes(it.value[2:])
                else:
                ...
                if modeEDefaultValues and self.client.interfaceType == InterfaceType.HDLC_WITH_MODE_E and isinstance(self.media, MILDSerial):
                ...
                MILDDLMSObject.validateLogicalName(self.invocationCounter)
            elif it.tag == 'g':
                ...
                if self.client.serverAddress != 1:
                    self.client.serverAddress = MILDDLMSClient.getServerAddress(self.client.serverAddress, int(it.value))
                else:
                    self.client.serverAddress = int(it.value)
            elif it.tag == 'l':
                self.client.serverAddress = MILDDLMSClient.getServerAddress(int(it.value), self.client.serverAddress)
            elif it.tag == 'n':
                self.client.serverAddress = MILDDLMSClient.getServerAddressFromSerialNumber(int(it.value))
            elif it.tag == 'W':
