...
from common_mt.io import Parity, StopBits
from common_mt import ReceiveParameters, MILDCommon, TimeoutException
from dlms_mt import MILDByteBuffer, MILDReplyData, MILDDLMSTranslator, MILDDLMSException, MILDDLMSAccessItem
from dlms_mt.enums import InterfaceType, ObjectType, Authentication, Conformance, DataType,\
    Security, AssociationResult, SourceDiagnostic, AccessServiceCommandType
from dlms_mt.objects import MILDDLMSObject, MILDDLMSObjectCollection, MILDDLMSData, MILDDLMSRegister,\
    MILDDLMSDemandRegister, MILDDLMSProfileGeneric, MILDDLMSExtendedRegister
from net_mt import MILDNet
from serial_mt import MILDSerial

class MILDDLMSReader:
    ...
    def readDLMSPacket2(self, data, reply):
        ...
        if self.client.interfaceType == InterfaceType.WRAPPER and isinstance(self.media, MILDNet):
            eop = None
        p = ReceiveParameters()
        p.eop = eop
        p.allData = True
    ...
    def initializeOpticalHead(self):
        if self.client.interfaceType == InterfaceType.HDLC_WITH_MODE_E:
            ...
            with self.media.getSynchronous():
                ...
                time.sleep(1)
                self.writeTrace("TX: " + self.now() + "\t" + MILDCommon.toHex(tmp), TraceLevel.VERBOSE)
                p.waitTime = 200
    ...
    def updateFrameCounter(self):
        if self.invocationCounter and self.client.ciphering is not None and self.client.ciphering.security != Security.NONE:
            ...
            try:
                ...
                reply.clear()
                d = MILDDLMSData(self.invocationCounter)
                self.read(d, 2)
    ...
    def initializeConnection(self):
        ...
        if self.client.ciphering.security != Security.NONE:
            ...
            print("System title: " + MILDCommon.toHex(self.client.ciphering.systemTitle))
            print("Authentication key: " + MILDCommon.toHex(self.client.ciphering.authenticationKey))
            print("Block cipher key: " + MILDCommon.toHex(self.client.ciphering.blockCipherKey))
            if self.client.ciphering.dedicatedKey:
                print("Dedicated key: " + MILDCommon.toHex(self.client.ciphering.dedicatedKey))
        ...
        if self.client.authentication > Authentication.LOW:
            ...
            except MILDDLMSException as ex:
                #Invalid password.
                raise MILDDLMSException(AssociationResult.PERMANENT_REJECTED, SourceDiagnostic.AUTHENTICATION_FAILURE)
    ...
    def readScalerAndUnits(self):
        ...
        if self.client.negotiatedConformance & Conformance.ACCESS != 0:
            for it in objs:
                if isinstance(it, (MILDDLMSRegister, MILDDLMSExtendedRegister)):
                    list_.append(MILDDLMSAccessItem(AccessServiceCommandType.GET, it, 3))
                elif isinstance(it, (MILDDLMSDemandRegister)):
                    list_.append(MILDDLMSAccessItem(AccessServiceCommandType.GET, it, 4))
        ...
        if self.client.negotiatedConformance & Conformance.MULTIPLE_REFERENCES == 0:
            for it in objs:
                try:
                    if isinstance(it, (MILDDLMSRegister,)):
                        self.read(it, 3)
                    elif isinstance(it, (MILDDLMSDemandRegister,)):
                        self.read(it, 4)
                except Exception:
    ...
    def getReadOut(self):
        ...
        for it in self.client.objects:
            if type(it) == MILDDLMSObject:
                print("Unknown Interface: " + it.objectType.__str__())
                continue
            if isinstance(it, MILDDLMSProfileGeneric):
                continue
            ...
            for pos in it.getAttributeIndexToRead(True):
                try:
                    ...
                    if not isinstance(ex, (MILDDLMSException, TimeoutException)):
                        traceback.print_exc()

    def showValue(self, pos, val):
        if isinstance(val, (bytes, bytearray)):
            val = MILDByteBuffer(val)
                    ...
                    for rows in cells:
                        for cell in rows:
                            if isinstance(cell, bytearray):
                                self.writeTrace(MILDByteBuffer.hex(cell) + " | ", TraceLevel.INFO)
                            else:
    ...
    def readAll(self, outputFile):
        try:
            ...
            if outputFile and os.path.exists(outputFile):
                print(f'_____________output file {outputFile}')
                try:
                    c = MILDDLMSObjectCollection.load(outputFile)
            ...
            #if outputFile:
            print('______final_saving the file',outputFile)
