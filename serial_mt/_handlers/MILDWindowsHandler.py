...
from .IMILDNative import IMILDNative
...
class MILDWindowsHandler(MILDSettings, IMILDNative):
    #is Windows unicode.
    __unicode = None

    def __init__(self):
        """Constructor."""
        MILDSettings.__init__(self)
    ...
    def isUnicode(self):
        ...
        if MILDWindowsHandler.__unicode is None:
            try:
                h = _stdcall_libraries['kernel32'].CreateEventW(0, 0, 0, 0)
                ctypes.windll.Kernel32.CloseHandle(h)
                MILDWindowsHandler.__unicode = True
            except AttributeError:
                MILDWindowsHandler.__unicode = False
        return MILDWindowsHandler.__unicode
