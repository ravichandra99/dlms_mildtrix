...
import threading
import traceback
import common_mt.io.BaudRate
import common_mt.io.Parity
import common_mt.io.StopBits
from common_mt.enums import TraceLevel, MediaState, TraceTypes
from common_mt.IMILDMedia import IMILDMedia
from common_mt.MediaStateEventArgs import MediaStateEventArgs
from common_mt.TraceEventArgs import TraceEventArgs
from common_mt.PropertyChangedEventArgs import PropertyChangedEventArgs
from common_mt.ReceiveEventArgs import ReceiveEventArgs
from ._MILDSynchronousMediaBase import _MILDSynchronousMediaBase
if os.name == 'nt':  # sys.platform == 'win32':
    from ._handlers.MILDWindowsHandler import MILDWindowsHandler
elif os.name == 'posix':
    from ._handlers.MILDLinuxHandler import MILDLinuxHandler

# pylint: disable=too-many-public-methods, too-many-instance-attributes,
# too-many-arguments
class MILDSerial(IMILDMedia):
    def __init__(self,
                 port,
                 baudRate=common_mt.io.BaudRate.BAUD_RATE_9600,
                 dataBits=8,
                 parity=common_mt.io.Parity.NONE,
                 stopBits=common_mt.io.StopBits.ONE):
        """Constructor.
        port : Serial port.
        baudRate : Baud rate.
        dataBits : Data bits.
        ...
        self.__syncBase = _MILDSynchronousMediaBase(100)
    ...
    def __initialize(cls):
        ###Initialize Mildtrix serial port library.
        if os.name == 'nt':
            return MILDWindowsHandler()
        if os.name == 'posix':
            return MILDLinuxHandler()
    ...
    def getAvailableBaudRates(self):
        ...
        return (common_mt.io.BaudRate.BAUD_RATE_300, common_mt.io.BaudRate.BAUD_RATE_600,
                common_mt.io.BaudRate.BAUD_RATE_1800, common_mt.io.BaudRate.BAUD_RATE_2400,
                common_mt.io.BaudRate.BAUD_RATE_4800, common_mt.io.BaudRate.BAUD_RATE_9600,
                common_mt.io.BaudRate.BAUD_RATE_19200, common_mt.io.BaudRate.BAUD_RATE_38400)
    ...
    def send(self, data, receiver=None):
        ...
        if not isinstance(data, bytes):
            data = bytes(_MILDSynchronousMediaBase.toBytes(data))
        self.__h.write(data)
    ...
    def __handleReceivedData(self, buff, info):
        ...
        if self.getIsSynchronous():
            ...
            with self.__syncBase.getSync():
                ...
                if self.eop:
                    tmp = _MILDSynchronousMediaBase.toBytes(self.eop)
                    totalCount = _MILDSynchronousMediaBase.indexOf(buff, tmp, 0, len(buff))
                if totalCount != -1:
    ...
    def __getBaudRate(self):
        if self.__h.isOpen():
            self.__h.baudRate = common_mt.io.BaudRate(self.__h.getBaudRate())
    ...
    def __getParity(self):
        if self.__h.isOpen():
            self.__h.parity = common_mt.io.Parity(self.__h.getParity())
    ...
    def __getStopBits(self):
        if self.__h.isOpen():
            self.__h.stopBits = common_mt.io.StopBits(self.__h.getStopBits())
    ...
    def getSettings(self):
        ...
        if self.__h.baudRate != common_mt.io.BaudRate.BAUD_RATE_9600:
            sb += "<BaudRate>"
            sb += str(int(self.__h.baudRate))
            sb += "</BaudRate>"
            sb += nl

        if self.__h.stopBits != common_mt.io.StopBits.ONE:
            sb += "<StopBits>"
            sb += str(int(self.__h.stopBits))
            sb += "</StopBits>"
            sb += nl

        if self.__h.parity != common_mt.io.Parity.NONE:
            sb += "<Parity>"
    ...
    def setSettings(self, value):
        ...
        self.__portName = ""
        self.__h.baudRate = common_mt.io.BaudRate.BAUD_RATE_9600
        self.__h.stopBits = common_mt.io.StopBits.ONE
        self.__h.parity = common_mt.io.Parity.NONE
        self.__h.dataBits = 8
