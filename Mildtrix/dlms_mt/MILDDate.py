...
class MILDDate(MILDDateTime):
    def __init__(self, value=None, pattern=None):
        ...
        value: Date value.
        """
        MILDDateTime.__init__(self, value, pattern)
    ...
    def _remove(self, format_):
        format_ = MILDDateTime._remove_(format_, "%H", True)
        format_ = MILDDateTime._remove_(format_, "%-H", True)
        format_ = MILDDateTime._remove_(format_, "%I", True)
        format_ = MILDDateTime._remove_(format_, "%-I", True)
        format_ = MILDDateTime._remove_(format_, "%M", True)
        format_ = MILDDateTime._remove_(format_, "%-M", True)
        format_ = MILDDateTime._remove_(format_, "%S", True)
        format_ = MILDDateTime._remove_(format_, "%-S", True)
        str_ = datetime.datetime.now().strftime('%p')
        format_ = MILDDateTime._remove_(format_, str_, True)
        return format_
