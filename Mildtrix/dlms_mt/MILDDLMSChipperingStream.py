...
class MILDDLMSChipperingStream:
    """
    ...
    def __init__(self, forSecurity, forEncrypt, blockCipherKey, forAad, iv, forTag):
        ...
        if self.encrypt:
            bufLength = MILDDLMSChipperingStream.BLOCK_SIZE
        else:
            bufLength = (MILDDLMSChipperingStream.BLOCK_SIZE + MILDDLMSChipperingStream.TAG_SIZE)
        self.bufBlock = bytearray(bufLength)
        self.aad = forAad
        self.h = bytearray(MILDDLMSChipperingStream.BLOCK_SIZE)
        if iv:
    ...
    def flushFinalBlock(self):
        ...
        if not self.encrypt:
            if not self.tagsEquals(self.tag, generatedTag):
                print(MILDByteBuffer.hex(self.tag, False) + "-" + MILDByteBuffer.hex(generatedTag, False))
