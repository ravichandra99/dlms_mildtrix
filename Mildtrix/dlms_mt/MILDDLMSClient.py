...
from .objects import MILDDLMSObject, MILDDLMSObjectCollection, MILDDLMSData
...
class MILDDLMSClient(object):
    ...
    def __init__(self, useLogicalNameReferencing=True, clientAddress=16, serverAddress=1, forAuthentication=Authentication.NONE, password=None, interfaceType=InterfaceType.HDLC):
        # DLMS settings.
        self.settings = MILDDLMSSettings(False)
        ...
        if not password:
            ...
            self.password = _MILDCommon.getBytes(password)
    ...
    #
    # @see MILDDLMSClient#getAuthentication
    # Used password.
    #
    ...
    #
    # @see MILDDLMSClient#getPassword
    # @see MILDDLMSClient#getClientAddress
    # Used authentication.
    #
    ...
    #
    # @see MILDDLMSClient#getClientAddress
    # @see MILDDLMSClient#getServerAddress
    # @see MILDDLMSClient#parseUAResponse
    ...
    def snrmRequest(self):
        ...
        if MILDHdlcSettings.DEFAULT_MAX_INFO_TX != self.hdlcSettings.maxInfoTX:
            data.setUInt8(_HDLCInfo.MAX_INFO_TX)
            MILDDLMS.appendHdlcParameter(data, self.hdlcSettings.maxInfoTX)
        if MILDHdlcSettings.DEFAULT_MAX_INFO_RX != self.hdlcSettings.maxInfoRX:
            data.setUInt8(_HDLCInfo.MAX_INFO_RX)
            MILDDLMS.appendHdlcParameter(data, self.hdlcSettings.maxInfoRX)
        if MILDHdlcSettings.DEFAULT_WINDOWS_SIZE_TX != self.hdlcSettings.windowSizeTX:
            data.setUInt8(_HDLCInfo.WINDOW_SIZE_TX)
            data.setUInt8(4)
            data.setUInt32(self.hdlcSettings.windowSizeTX)
        if MILDHdlcSettings.DEFAULT_WINDOWS_SIZE_RX != self.hdlcSettings.windowSizeRX:
        ...
        if data.size != 3:
            ...
            data = None
        return MILDDLMS.getHdlcFrame(self.settings, Command.SNRM, data)

    #
    # Parses UAResponse from byte array.
    #
    # data: # Received message from the server.
    # @see MILDDLMSClient#snrmRequest
    #
    def parseUAResponse(self, data):
        if not isinstance(data, MILDByteBuffer):
            data = MILDByteBuffer(data)

        MILDDLMS.parseSnrmUaResponse(data, self.settings.hdlc)
    ...
    # @see MILDDLMSClient#parseAareResponse
    #
    def aarqRequest(self):
        ...
        buff = MILDByteBuffer(20)
        self.settings.resetBlockIndex()
        MILDDLMS.checkInit(self.settings)
        ...
        if self.authentication > Authentication.LOW:
            if not self.settings.useCustomChallenge:
                self.settings.ctoSChallenge = MILDSecure.generateChallenge()
        else:
            self.settings.setCtoSChallenge(None)
        _MILDAPDU.generateAarq(self.settings, self.settings.cipher, None, buff)
        reply = None
        if self.settings.getUseLogicalNameReferencing():
            p = MILDDLMSLNParameters(self.settings, 0, Command.AARQ, 0, buff, None, 0xff)
            reply = MILDDLMS.getLnMessages(p)
        else:
            p = MILDDLMSSNParameters(self.settings, Command.AARQ, 0, 0, None, buff)
    ...
    # reply
    # Received data.
    # @see MILDDLMSClient#aarqRequest
    # @see MILDDLMSClient#useLogicalNameReferencing
    # @see MILDDLMSClient#negotiatedConformance
    # @see MILDDLMSClient#proposedConformance
    #
    def parseAareResponse(self, reply):
        self.isAuthenticationRequired = _MILDAPDU.parsePDU(self.settings, self.settings.cipher, reply, None) == SourceDiagnostic.AUTHENTICATION_REQUIRED
    ...
    def getApplicationAssociationRequest(self):
        ...
        challenge = MILDSecure.secure(self.settings, self.settings.cipher, self.settings.cipher.invocationCounter, self.settings.getStoCChallenge(), pw)
        if self.useLogicalNameReferencing:
            return self.__method("0.0.40.0.0.255", ObjectType.ASSOCIATION_LOGICAL_NAME, 1, challenge, DataType.OCTET_STRING)
        return self.__method(0xFA00, ObjectType.ASSOCIATION_SHORT_NAME, 8, challenge, DataType.OCTET_STRING)
    ...
    def parseApplicationAssociationResponse(self, reply):
        ...
        if self.manufacturerId == "LGZ" and self.settings.authentication == Authentication.HIGH:
            ...
            equals = False
            ic = 0
            value = _MILDCommon.getData(self.settings, reply, info)
            if value:
                ...
                tmp = MILDSecure.secure(self.settings, self.settings.cipher, ic, self.settings.getCtoSChallenge(), secret)
                challenge = MILDByteBuffer(tmp)
                equals = challenge.compare(value)
                if not equals:
                    print("Invalid StoC:" + MILDByteBuffer.hex(value, True) + "-" + MILDByteBuffer.hex(tmp, True))
            else:
    ...
    def releaseRequest(self):
        ...
        if self.useProtectedRelease:
            ...
            _MILDAPDU.generateUserInformation(self.settings, self.settings.cipher, None, buff)
        ...
        if self.useLogicalNameReferencing:
            p = MILDDLMSLNParameters(self.settings, 0, Command.RELEASE_REQUEST, 0, buff, None, 0xff)
            reply = MILDDLMS.getLnMessages(p)
        else:
            reply = MILDDLMS.getSnMessages(MILDDLMSSNParameters(self.settings, Command.RELEASE_REQUEST, 0xFF, 0xFF, None, buff))
        self.settings.connected = self.settings.connected & ~ConnectionState.DLMS
    ...
    def disconnectRequest(self, force=False):
        if not force and self.settings.connected == ConnectionState.NONE:
            return None
        if MILDDLMS.useHdlc(self.interfaceType):
            self.settings.connected = ConnectionState.NONE
            reply = MILDDLMS.getHdlcFrame(self.settings, Command.DISCONNECT_REQUEST, None)
    ...
    def __createDLMSObject(cls, classID, version, baseName, ln, accessRights):
        ...
        MILDDLMSClient.__updateObjectData(obj, type_, version, baseName, ln, accessRights)
        return obj
    ...
    def parseSNObjects(self, buff, onlyKnownObjects, ignoreInactiveObjects):
        ...
        items = MILDDLMSObjectCollection(self)
        cnt = _MILDCommon.getObjectCount(buff)
        info = _MILDDataInfo()
        objPos = 0
        while objPos != cnt:
            ...
            objects = _MILDCommon.getData(self.settings, buff, info)
            if len(objects) != 4:
                raise Exception("Invalid structure format.")
            classID = objects[1]
            baseName = int(objects[0]) & 0xFFFF
            comp = MILDDLMSClient.__createDLMSObject(classID, objects[2], baseName, objects[3], None)
            if (not onlyKnownObjects or type(comp) != MILDDLMSObject):
    ...
    def __updateObjectData(cls, obj, objectType, version, baseName, logicalName, accessRights):
        ...
        obj.logicalName = _MILDCommon.toLogicalName(logicalName)
    ...
    def parseObjects(self, data, onlyKnownObjects=True, ignoreInactiveObjects=True):
        ...
        c = MILDDLMSConverter(self.standard)
    ...
    def parseLNObjects(self, buff, onlyKnownObjects, ignoreInactiveObjects):
        ...
        while objPos != cnt:
            ...
            classID = objects[0]
            if classID > 0:
                comp = MILDDLMSClient.__createDLMSObject(classID, objects[1], 0, objects[2], objects[3])
                if (not onlyKnownObjects or type(comp) != MILDDLMSObject):
                    if not ignoreInactiveObjects or comp.logicalName != "0.0.127.0.0.0":
                        items.append(comp)
                else:
                    print("Unknown object : " + str(classID) + " " + _MILDCommon.toLogicalName(objects[2]))
            objPos += 1
        return items
    ...
    def getValue(cls, data, useUtc):
        ...
        info = _MILDDataInfo()
        return _MILDCommon.getData(settings, data, info)
    ...
    def changeType(cls, value, type_, useUtc=False):
        settings = MILDDLMSSettings(False)
        settings.useUtc2NormalTime = useUtc
        return _MILDCommon.changeType(settings, value, type_)
    ...
    def __method(self, name, objectType, methodIndex, value, dataType=DataType.NONE):
        ...
        reply = None
        data = MILDByteBuffer()
        attributeDescriptor = MILDByteBuffer()
        _MILDCommon.setData(self.settings, data, type_, value)
        if self.useLogicalNameReferencing:
            attributeDescriptor.setUInt16(objectType)
            attributeDescriptor.set(_MILDCommon.logicalNameToBytes(str(name)))
            attributeDescriptor.setUInt8(int(methodIndex))
            if type_ == DataType.NONE:
                attributeDescriptor.setUInt8(0)
            else:
                attributeDescriptor.setUInt8(1)
            p = MILDDLMSLNParameters(self.settings, 0, Command.METHOD_REQUEST, ActionRequestType.NORMAL, attributeDescriptor, data, 0xff)
            reply = MILDDLMS.getLnMessages(p)
        else:
            ind = [0]
            count = [0]
            MILDDLMS.getActionInfo(objectType, ind, count)
            if index > count[0]:
            ...
            if type_ != DataType.NONE:
                ...
                attributeDescriptor.setUInt8(0)
            p = MILDDLMSSNParameters(self.settings, Command.WRITE_REQUEST, 1, VariableAccessSpecification.VARIABLE_NAME, attributeDescriptor, data)
            reply = MILDDLMS.getSnMessages(p)
        return reply
    ...
    def __write(self, name, value, dataType, objectType, index):
        ...
        if self.useLogicalNameReferencing:
            ...
            attributeDescriptor.setUInt8(0)
            p = MILDDLMSLNParameters(self.settings, 0, Command.SET_REQUEST, SetRequestType.NORMAL, attributeDescriptor, data, 0xff)
    ...
    def writeList(self, list_):
        ...
        if self.useLogicalNameReferencing:
            ...
            for it in list_:
                ...
                bb.set(_MILDCommon.logicalNameToBytes(it.target.logicalName))
            ...
            for it in list_:
                ...
                sn += (it.index - 1) * 8
                bb.setUInt16(sn)
        _MILDCommon.setObjectCount(len(list_), bb)
        for it in list_:
            ...
            _MILDCommon.setData(self.settings, data, type_, value)
        if self.useLogicalNameReferencing:
            p = MILDDLMSLNParameters(self.settings, 0, Command.SET_REQUEST, SetRequestType.WITH_LIST, bb, data, 0xff)
            reply = MILDDLMS.getLnMessages(p)
        else:
            p2 = MILDDLMSSNParameters(self.settings, Command.WRITE_REQUEST, len(list_), 4, bb, data)
    ...
    def _read(self, name, objectType, attributeOrdinal, data=None):
        ...
        if self.useLogicalNameReferencing:
            ...
            if not data:
                ...
                attributeDescriptor.setUInt8(1)
            p = MILDDLMSLNParameters(self.settings, 0, Command.GET_REQUEST, GetCommandType.NORMAL, attributeDescriptor, data, 0xFF)
            reply = MILDDLMS.getLnMessages(p)
        else:
            #pylint: disable=bad-option-value,redefined-variable-type
            sn = name
            ...
            p = MILDDLMSSNParameters(self.settings, Command.READ_REQUEST, 1, requestType, attributeDescriptor, data)
            reply = MILDDLMS.getSnMessages(p)
        return reply
    ...
    def readList(self, list_):
        ...
        self.settings.resetBlockIndex()
        if self.useLogicalNameReferencing:
            p = MILDDLMSLNParameters(self.settings, 0, Command.GET_REQUEST, GetCommandType.WITH_LIST, data, None, 0xff)
            pos = 0
            ...
            if count > 10:
                count = 10
            _MILDCommon.setObjectCount(count, data)
            for k, v in list_:
                data.setUInt16(k.objectType)
                data.set(_MILDCommon.logicalNameToBytes(k.logicalName))
                data.setUInt8(v)
                data.setUInt8(0)
                pos += 1
                if pos % count == 0 and len(list_) != pos:
                    messages.append(MILDDLMS.getLnMessages(p))
                    data.clear()
                    if len(list_) - pos < count:
                        _MILDCommon.setObjectCount(len(list_) - pos, data)
                    else:
                        _MILDCommon.setObjectCount(count, data)
            messages.append(MILDDLMS.getLnMessages(p))
        else:
            p2 = MILDDLMSSNParameters(self.settings, Command.READ_REQUEST, len(list_), 0xFF, data, None)
            for k, v in list_:
                data.setUInt8(VariableAccessSpecification.VARIABLE_NAME)
                sn = k.shortName
                sn += (v - 1) * 8
                data.setUInt16(sn)
            messages.append(MILDDLMS.getSnMessages(p2))
        return messages

    def keepAlive(self):
        if self.interfaceType == InterfaceType.WRAPPER:
            return None
        return MILDDLMS.getHdlcFrame(self.settings, self.settings.getReceiverReady(), None)
    ...
    def readRowsByEntry(self, pg, index, count, columns=None):
        ...
        pg.buffer = list()
        buff = MILDByteBuffer(19)
        buff.setUInt8(0x02)
        buff.setUInt8(DataType.STRUCTURE)
        buff.setUInt8(0x04)
        _MILDCommon.setData(self.settings, buff, DataType.UINT32, index)
        if count == 0:
            _MILDCommon.setData(self.settings, buff, DataType.UINT32, count)
        else:
            _MILDCommon.setData(self.settings, buff, DataType.UINT32, index + count - 1)
        columnIndex = 1
        columnCount = 0
        pos = 0
        if columns:
        ...
        _MILDCommon.setData(self.settings, buff, DataType.UINT16, columnIndex)
        _MILDCommon.setData(self.settings, buff, DataType.UINT16, columnCount)
        return self._read(pg.name, ObjectType.PROFILE_GENERIC, 2, buff)

    def readRowsByRange(self, pg, start, end, columns=None):
        pg.buffer = list()
        self.settings.resetBlockIndex()
        if not isinstance(start, MILDDateTime):
            start = MILDDateTime(start)
        if not isinstance(end, MILDDateTime):
            end = MILDDateTime(end)
        sort = pg.sortObject
        if not sort and pg.captureObjects:
            sort = pg.captureObjects[0][0]
        buff = MILDByteBuffer(51)
        buff.setUInt8(0x01)
        buff.setUInt8(DataType.STRUCTURE)
        buff.setUInt8(0x04)
        buff.setUInt8(DataType.STRUCTURE)
        ...
        if sort:
            ...
            ot = ObjectType.CLOCK
            ln = "0.0.1.0.0.255"
        _MILDCommon.setData(self.settings, buff, DataType.UINT16, ot)
        _MILDCommon.setData(self.settings, buff, DataType.OCTET_STRING, _MILDCommon.logicalNameToBytes(ln))
        _MILDCommon.setData(self.settings, buff, DataType.INT8, 2)
        _MILDCommon.setData(self.settings, buff, DataType.UINT16, 0)
        if sort and isinstance(sort, MILDDLMSData) and sort.logicalName == "0.0.1.1.0.255":
            _MILDCommon.setData(self.settings, buff, DataType.UINT32, MILDDateTime.toUnixTime(start))
            _MILDCommon.setData(self.settings, buff, DataType.UINT32, MILDDateTime.toUnixTime(end))
        else:
            _MILDCommon.setData(self.settings, buff, DataType.OCTET_STRING, start)
            _MILDCommon.setData(self.settings, buff, DataType.OCTET_STRING, end)
        buff.setUInt8(DataType.ARRAY)
        if not columns:
            buff.setUInt8(0x00)
        else:
            _MILDCommon.setObjectCount(len(columns), buff)
            for it in columns:
                buff.setUInt8(DataType.STRUCTURE)
                buff.setUInt8(4)
                _MILDCommon.setData(self.settings, buff, DataType.UINT16, it[0].objectType)
                _MILDCommon.setData(self.settings, buff, DataType.OCTET_STRING, _MILDCommon.logicalNameToBytes(it[0].logicalName))
                _MILDCommon.setData(self.settings, buff, DataType.INT8, it[1].attributeIndex)
                _MILDCommon.setData(self.settings, buff, DataType.UINT16, it[1].dataIndex)
        return self._read(pg.name, ObjectType.PROFILE_GENERIC, 2, buff)

    @classmethod
    def createObject(cls, type_):
        return _MILDObjectFactory.createObject(type_)

    def receiverReady(self, type_):
        return MILDDLMS.receiverReady(self.settings, type_)
    ...
    def getData(self, reply, data, notify=None):
        ...
        if isinstance(reply, bytearray):
            reply = MILDByteBuffer(reply)
        elif isinstance(reply, bytes):
            reply = MILDByteBuffer(reply)
        try:
            ret = MILDDLMS.getData(self.settings, reply, data, notify)
        ...
        if ret and self.translator and data.moreData == RequestTypes.NONE:
            if data.xml is None:
                data.xml = (MILDDLMSTranslatorStructure(self.translator.outputType, self.translator.omitXmlNameSpace, self.translator.hex, self.translator.showStringAsHex, self.translator.comments, self.translator.tags))
            pos = data.data.position
            try:
                data2 = data.data
                if data.command == Command.GET_RESPONSE:
                    tmp = MILDByteBuffer((4 + data.data.size))
                    ...
                    tmp = MILDByteBuffer((6 + data.data.size))
                    tmp.setUInt8(data.command)
                    tmp.setUInt8(GetCommandType.NORMAL)
                    tmp.setUInt8(int(data.invokeId))
                    tmp.setUInt8(0)
                    ...
                    tmp = MILDByteBuffer(3 + data.data.size)
                    tmp.setUInt8(data.command)
    ...
    def accessRequest(self, time, list_):
        ...
        for it in list_:
            if it.command == AccessServiceCommandType.GET:
                ...
                if type_ == DataType.NONE:
                    raise Exception("Invalid parameter. In python value type must give.")
                _MILDCommon.setData(self.settings, bb, type_, value)
            else:
                raise ValueError("Invalid command.")

        p = MILDDLMSLNParameters(self.settings, 0, Command.ACCESS_REQUEST, 0xFF, None, bb, 0xff)
        if time:
            p.time = MILDDateTime(time)
        return MILDDLMS.getLnMessages(p)

    def parseAccessResponse(self, list_, data):
        info = _MILDDataInfo()
        cnt = _MILDCommon.getObjectCount(data)
        if len(list_) != cnt:
            raise ValueError("List size and values size do not match.")
        for it in list_:
            info.clear()
            it.value = _MILDCommon.getData(self.settings, data, info)
        cnt = _MILDCommon.getObjectCount(data)
        if len(list_) != cnt:
            raise ValueError("List size and values size do not match.")
        for it in list_:
    ...
    def getInitialConformance(cls, useLogicalNameReferencing):
        return MILDDLMSSettings.getInitialConformance(useLogicalNameReferencing)

    def parseReport(self, reply, list_):
        if reply.command == Command.EVENT_NOTIFICATION:
            MILDDLMSLNCommandHandler.handleEventNotification(self.settings, reply, list_)
            return None
        if reply.command == Command.INFORMATION_REPORT:
            MILDDLMSSNCommandHandler.handleInformationReport(self.settings, reply, list_)
            return None
    ...
    def parsePushObjects(self, data):
        ...
        if data:
            ...
            for it in data:
                ...
                if classID > 0:
                    comp = None
                    comp = self.objects.findByLN(classID, _MILDCommon.toLogicalName(tmp[1]))
                    if comp is None:
                        comp = MILDDLMSClient.__createDLMSObject(classID, 0, 0, tmp[1], None)
                        self.settings.objects.append(comp)
                        c.updateOBISCodeInformation(comp)
                    if comp.__class__ != MILDDLMSObject.__class__:
                        objects.append((comp, tmp[2]))
                    else:
                        print("Unknown object: " + str(classID) + " " + _MILDCommon.toLogicalName(tmp[1]))
        return objects
