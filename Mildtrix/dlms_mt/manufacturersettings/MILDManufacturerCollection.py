...
from .MILDManufacturer import MILDManufacturer
from .MILDObisCode import MILDObisCode
from .MILDAuthentication import MILDAuthentication
from .MILDServerAddress import MILDServerAddress
from .MILDDLMSAttribute import MILDDLMSAttribute
...
#
class MILDManufacturerCollection(list):
    #
    ...
    #
    # Check if there are any updates available in Mildtrix www server.
    #
    # @param path
    #            Settings directory.
    ...
    def isUpdatesAvailable(cls, path):
        ...
        try:
            ...
            path.close()
            urllib.request.urlretrieve("https://www.mildtrix.fi/obis/files.xml", path.name)
    ...
    def updateManufactureSettings(cls, directory):
        #
        # Update manufacturer settings from the Mildtrix www server.
        #
        ...
        urllib.request.urlretrieve("https://www.mildtrix.fi/obis/files.xml", path)
        for it in ET.parse(path).iter():
            if it.tag == "File":
                path = os.path.join(directory, it.text)
                urllib.request.urlretrieve("https://www.mildtrix.fi/obis/" + it.text, path)
    ...
    def readManufacturerSettings(cls, manufacturers, path):
        ...
        if files:
            for it in files:
                if it.endswith(".obx"):
                    try:
                        manufacturers.append(MILDManufacturerCollection.__parse(os.path.join(path, it)))
                    except Exception as e:
                        print(e)
                        continue
    ...
    def __parse(cls, file):
        ...
        for it in ET.parse(file).iter():
            if it.tag == "MILDManufacturer":
                man = MILDManufacturer()
            elif it.tag == "MILDObisCode":
                obisCode = MILDObisCode()
                man.obisCodes.append(obisCode)
            elif it.tag == "MILDAuthentication":
                authentication = MILDAuthentication()
                man.settings.append(authentication)
            elif it.tag == "MILDServerAddress":
                serveraddress = MILDServerAddress()
                man.serverSettings.append(serveraddress)
            elif it.tag == "MILDDLMSAttributeSettings":
                att = MILDDLMSAttribute()
