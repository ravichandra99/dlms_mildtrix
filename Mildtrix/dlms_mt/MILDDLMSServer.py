...
from .objects.IMILDDLMSBase import IMILDDLMSBase
...
class MILDDLMSServer(object):
    ...
    def __init__(self, logicalNameReferencing, interfaceType):
        self.info = MILDReplyData()
        # Received data.
        self.receivedData = MILDByteBuffer()
        # Reply data.
        self.replyData = MILDByteBuffer()
 ...
 #
    ...
    def initialize(self):
        ...
        while pos != len(self.settings.objects):
            ...
            it.start = self
            if isinstance(it, (MILDDLMSAssociationShortName,)) and not self.useLogicalNameReferencing:
                if len(it.objectList) == 0:
                    it.objectList.append(self.items)
                associationObject = it
            elif isinstance(it, (MILDDLMSAssociationLogicalName,)) and self.useLogicalNameReferencing:
                ln = it
            ...
            elif not isinstance(it, IMILDDLMSBase):
                self.settings.objects.remove(pos)
                pos -= 1
            pos += 1
        if not associationObject:
            if self.useLogicalNameReferencing:
                it = MILDDLMSAssociationLogicalName()
                ...
                it2 = MILDDLMSAssociationShortName()
    ...
    def updateShortNames(self, force):
        ...
        for it in self.settings.objects:
            if not isinstance(it, (MILDDLMSAssociationShortName, MILDDLMSAssociationLogicalName)):
                if force or it.shortName == 0:
                    it.setShortName(sn)
                    MILDDLMS.getActionInfo(it.objectType, offset, count)
                    if count[0] != 0:
    ...
    def handleAarqRequest(self, data, connectionInfo):
        ...
        except MILDDLMSConfirmedServiceError as e:
            result = AssociationResult.PERMANENT_REJECTED
            diagnostic = SourceDiagnostic.NO_REASON_GIVEN
            error = MILDByteBuffer()
            error.setUInt8(0xE)
        ...
        except MILDDLMSException as e:
            result = e.result
            diagnostic = e.diagnostic
        if self.settings.interfaceType == InterfaceType.HDLC or self.settings.interfaceType == InterfaceType.HDLC_WITH_MODE_E:
            self.replyData.set(_MILDCommon.LLC_REPLY_BYTES)
        if self.settings.authentication > Authentication.LOW:
            self.settings.setStoCChallenge(MILDSecure.generateChallenge())
        _MILDAPDU.generateAARE(self.settings, self.replyData, result, diagnostic, self.settings.cipher, error, None)

    def handleReleaseRequest(self, data):
        # pylint: disable=unused-argument
        if self.settings.interfaceType == InterfaceType.HDLC or self.settings.interfaceType == InterfaceType.HDLC_WITH_MODE_E:
            self.replyData.set(0, _MILDCommon.LLC_REPLY_BYTES)
        tmp = _MILDAPDU.getUserInformation(self.settings, self.settings.cipher)
    ...
    def handleSnrmRequest(self, data):
        if self.hdlc:
            ...
            self.settings.hdlc.maxInfoRX = MILDHdlcSettings.DEFAULT_MAX_INFO_RX
            self.settings.hdlc.maxInfoTX = MILDHdlcSettings.DEFAULT_MAX_INFO_TX
            self.settings.hdlc.windowSizeRX = MILDHdlcSettings.DEFAULT_WINDOWS_SIZE_RX
            self.settings.hdlc.windowSizeTX = MILDHdlcSettings.DEFAULT_WINDOWS_SIZE_TX
        ...
        MILDDLMS.appendHdlcParameter(self.replyData, self.hdlc.maxInfoTX)
        self.replyData.setUInt8(_HDLCInfo.MAX_INFO_RX)
        MILDDLMS.appendHdlcParameter(self.replyData, self.hdlc.maxInfoRX)
    ...
    def handleRequest(self, sr):
        ...
        try:
            if not sr.isStreaming():
                ...
                try:
                    MILDDLMS.getData(self.settings, self.receivedData, self.info, None)
                except Exception:
                    self.dataReceived = datetime.datetime.now()
                    self.receivedData.size = 0
                    sr.setReply(MILDDLMS.getHdlcFrame(self.settings, Command.UNACCEPTABLE_FRAME, self.replyData))
                    return
                ...
                if self.info.command == Command.DISCONNECT_REQUEST and (self.settings.connected == ConnectionState.NONE):
                    sr.setReply(MILDDLMS.getHdlcFrame(self.settings, Command.DISCONNECT_MODE, self.replyData))
                    self.info.clear()
                    return
                ...
                if (self.info.moreData & RequestTypes.FRAME) == RequestTypes.FRAME:
                    self.dataReceived = datetime.datetime.now()
                    sr.setReply(MILDDLMS.getHdlcFrame(self.settings, self.settings.getReceiverReady(), self.replyData))
                    return
            ...
            if isinstance(e, (MILDDLMSConfirmedServiceError,)):
    ...
    def reportConfirmedServiceError(self, e):
        self.replyData.clear()
        if self.settings.interfaceType == InterfaceType.HDLC or self.settings.interfaceType == InterfaceType.HDLC_WITH_MODE_E:
            MILDDLMS.addLLCBytes(self.settings, self.replyData)
        ...
        if self.settings.interfaceType == InterfaceType.WRAPPER:
            return MILDDLMS.getWrapperFrame(self.settings, Command.CONFIRMED_SERVICE_ERROR, self.replyData)
        return MILDDLMS.getHdlcFrame(self.settings, int(0), self.replyData)
    ...
    def reportError(self, command, error):
        ...
        if self.settings.getUseLogicalNameReferencing():
            p = MILDDLMSLNParameters(self.settings, 0, cmd, 1, None, None, error)
            MILDDLMS.getLNPdu(p, self.replyData)
        else:
            bb = MILDByteBuffer()
            bb.setUInt8(error)
            p2 = MILDDLMSSNParameters(self.settings, cmd, 1, 1, None, bb)
            MILDDLMS.getSNPdu(p2, self.replyData)
        if self.settings.interfaceType == InterfaceType.WRAPPER:
            return MILDDLMS.getWrapperFrame(self.settings, command, self.replyData)
        return MILDDLMS.getHdlcFrame(self.settings, int(0), self.replyData)
    ...
    def handleCommand(self, cmd, data, sr):
        ...
        if cmd == Command.ACCESS_REQUEST:
            MILDDLMSLNCommandHandler.handleAccessRequest(self.settings, self, data, self.replyData, None)
        elif cmd == Command.SET_REQUEST:
            MILDDLMSLNCommandHandler.handleSetRequest(self.settings, self, data, self.replyData, None)
        elif cmd == Command.WRITE_REQUEST:
            MILDDLMSSNCommandHandler.handleWriteRequest(self.settings, self, data, self.replyData, None)
        elif cmd == Command.GET_REQUEST:
            if data:
                MILDDLMSLNCommandHandler.handleGetRequest(self.settings, self, data, self.replyData, None)
        elif cmd == Command.READ_REQUEST:
            MILDDLMSSNCommandHandler.handleReadRequest(self.settings, self, data, self.replyData, None)
        elif cmd == Command.METHOD_REQUEST:
            MILDDLMSLNCommandHandler.handleMethodRequest(self.settings, self, data, sr.getConnectionInfo(), self.replyData, None)
        ...
        if self.settings.interfaceType == InterfaceType.WRAPPER:
            reply = MILDDLMS.getWrapperFrame(self.settings, cmd, self.replyData)
        else:
            reply = MILDDLMS.getHdlcFrame(self.settings, frame_, self.replyData)
    ...
    def handleGeneralBlockTransfer(self, data, sr):
        if self.transaction:
            if self.transaction.command == Command.GET_REQUEST:
                ...
                MILDDLMSLNCommandHandler.getRequestNextDataBlock(self.settings, 0, self, data, self.replyData, None, True)
                if sr.count != 0:
                ...
                len_ = _MILDCommon.getObjectCount(data)
            ...
            if len_ > len(data) - data.position:
                ...
                self.transaction = MILDDLMSLongTransaction(None, data.getUInt8(), data)
                self.replyData.setUInt8(Command.GENERAL_BLOCK_TRANSFER)
