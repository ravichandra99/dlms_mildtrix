...
class MILDXmlReader:
        ...
        if name == self.name():
            ...
            if tp in (DataType.ARRAY, DataType.STRUCTURE):
                ...
                if uiType == DataType.OCTET_STRING:
                    ret = MILDByteBuffer.hexToBytes(str_)
                elif uiType == DataType.DATETIME:
                    ret = MILDDateTime(str_, "%m/%d/%Y %H:%M:%S")
                elif uiType == DataType.DATE:
                    ret = MILDDate(str_, "%m/%d/%Y")
                elif uiType == DataType.TIME:
                    ret = MILDTime(str_, "%H:%M:%S")
                elif uiType == DataType.NONE:
                    ret = str_
                else:
                    ret = MILDDLMSConverter.changeType(str_, uiType)
                self.getNext()
            return ret
        return defaultValue
    ...
    def readElementContentAsDateTime(self, name):
        if name == self.currentElement.tag:
            ...
            self.getNext()
            if str_:
                return MILDDateTime(str_, "%m/%d/%Y %H:%M:%S")
        return None
