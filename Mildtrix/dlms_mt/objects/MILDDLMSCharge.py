...
class MILDDLMSCharge(MILDDLMSObject, IMILDDLMSBase):
    """
    Online help:
    ...
    def __init__(self, ln="0.0.19.20.0.255", sn=0):
        ...
        """
        MILDDLMSObject.__init__(self, ObjectType.CHARGE, ln, sn)
        ...
        self.proportion = 0
        self.unitChargeActive = MILDUnitCharge()
        self.unitChargePassive = MILDUnitCharge()
    ...
    def getUIDataType(self, index):
        ...
        return super(MILDDLMSCharge, self).getUIDataType(index)
    ...
    def getUnitCharge(cls, settings, charge):
        ...
        bb.setUInt8(DataType.STRUCTURE)
        bb.setUInt8(2)
        _MILDCommon.setData(settings, bb, DataType.INT8, charge.chargePerUnitScaling.commodityScale)
        _MILDCommon.setData(settings, bb, DataType.INT8, charge.chargePerUnitScaling.priceScale)
        bb.setUInt8(DataType.STRUCTURE)
        bb.setUInt8(3)
        if charge.commodity.target is None:
            _MILDCommon.setData(settings, bb, DataType.UINT16, 0)
            ...
            bb.setUInt8(0)
            bb.setUInt8(0)
            _MILDCommon.setData(settings, bb, DataType.INT8, 0)
        else:
            _MILDCommon.setData(settings, bb, DataType.UINT16, charge.commodity.target.objectType)
            _MILDCommon.setData(settings, bb, DataType.OCTET_STRING, _MILDCommon.logicalNameToBytes(charge.commodity.target.logicalName))
            _MILDCommon.setData(settings, bb, DataType.INT8, charge.commodity.index)
        bb.setUInt8(DataType.ARRAY)
        if charge.chargeTables is None:
            bb.setUInt8(0)
        else:
            _MILDCommon.setObjectCount(len(charge.chargeTables), bb)
            for it in charge.chargeTables:
                bb.setUInt8(DataType.STRUCTURE)
                bb.setUInt8(2)
                _MILDCommon.setData(settings, bb, DataType.OCTET_STRING, it.index)
                _MILDCommon.setData(settings, bb, DataType.INT16, it.chargePerUnit)
        return bb.array()
    ...
    def getValue(self, settings, e):
        ...
        if e.index == 1:
            ...
            ret = MILDBitString.toBitString(self.chargeConfiguration, 2)
        elif e.index == 10:
    ...
    def setUnitCharge(cls, settings, charge, value):
        ...
        tmp2 = tmp[1]
        ot = tmp2[0]
        ln = _MILDCommon.toLogicalName(tmp2[1])
    ...
    def setValue(self, settings, e):
        if e.index == 1:
            ...
            if isinstance(e.value, bytearray):
                self.unitChargeActivationTime = _MILDCommon.changeType(settings, e.value, DataType.DATETIME)
            else:
            ...
            if isinstance(e.value, bytearray):
                self.lastCollectionTime = _MILDCommon.changeType(settings, e.value, DataType.DATETIME)
            else:
