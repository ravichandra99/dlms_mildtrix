...
class MILDDLMSRegisterActivation(MILDDLMSObject, IMILDDLMSBase):
    """
    Online help:
    http://www.mildtrix.fi/Mildtrix.DLMS.Objects.MILDDLMSRegisterActivation
    """
    ...
    def __init__(self, ln=None, sn=0):
        super(MILDDLMSRegisterActivation, self).__init__(ObjectType.REGISTER_ACTIVATION, ln, sn)
    ...
    def getValue(self, settings, e):
        ...
        if e.index == 3:
            ...
            for k, v in self.maskList:
                data.setUInt8(DataType.STRUCTURE)
                data.setUInt8(2)
                _MILDCommon.setData(settings, data, DataType.OCTET_STRING, k)
                data.setUInt8(DataType.ARRAY)
                data.setUInt8(len(v))
                for b in v:
                    _MILDCommon.setData(settings, data, DataType.UINT8, b)
            return data
        if e.index == 4:
    ...
    def setValue(self, settings, e):
        if e.index == 1:
            ...
            if e.value:
                for it in e.value:
                    item = MILDDLMSObjectDefinition()
                    item.objectType = ObjectType(it[0])
                    item.logicalName = _MILDCommon.toLogicalName(it[1])
    ...
    def load(self, reader):
        ...
        if reader.isStartElement("MaskList", True):
            while reader.isStartElement("Item", True):
                mask = MILDByteBuffer.hexToBytes(reader.readElementContentAsString("Mask"))
                str_ = reader.readElementContentAsString("Index")
                i = MILDByteBuffer.hexToBytes(str_.replace(";", " "))
                self.maskList.append((mask, i))
            reader.readEndElement("MaskList")
        self.activeMask = MILDByteBuffer.hexToBytes(reader.readElementContentAsString("ActiveMask"))
    ...
    def save(self, writer):
        ...
        if self.maskList:
            ...
            for k, v in self.maskList:
                writer.writeStartElement("Item")
                writer.writeElementString("Mask", MILDByteBuffer.hex(k))
                writer.writeElementString("Index", MILDByteBuffer.hex(v).replace(" ", ";"))
                writer.writeEndElement()
            writer.writeEndElement()
        if self.activeMask:
            writer.writeElementString("ActiveMask", MILDByteBuffer.hex(self.activeMask))
