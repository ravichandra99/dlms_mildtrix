...
class MILDDLMSAutoConnect(MILDDLMSObject, IMILDDLMSBase):
    """
    Online help:
    http://www.mildtrix.fi/Mildtrix.DLMS.Objects.MILDDLMSAutoConnect
    """
    ...
    def __init__(self, ln="0.0.2.1.0.255", sn=0):
        ...
        """
        MILDDLMSObject.__init__(self, ObjectType.AUTO_CONNECT, ln, sn)
    ...
    def getValue(self, settings, e):
        ...
        if e.index == 1:
            ...
            #  Add count
            _MILDCommon.setObjectCount(len(self.callingWindow), data)
            for k, v in self.callingWindow:
                ...
                #  End time
                _MILDCommon.setData(settings, data, DataType.OCTET_STRING, v)
            ret = data.array()
        elif e.index == 6:
            data = MILDByteBuffer()
            data.setUInt8(DataType.ARRAY)
            if not self.destinations:
                ...
                #  Add count
                _MILDCommon.setObjectCount(len(self.destinations), data)
                #  destination
                for it in self.destinations:
                    _MILDCommon.setData(settings, data, DataType.OCTET_STRING, _MILDCommon.getBytes(it))
            ret = data.array()
        else:
            e.error = ErrorCode.READ_WRITE_DENIED
        return ret
    ...
    def setValue(self, settings, e):
        if e.index == 1:
            ...
            if e.value:
                for item in e.value:
                    start = _MILDCommon.changeType(settings, item[0], DataType.DATETIME)
                    end = _MILDCommon.changeType(settings, item[1], DataType.DATETIME)
                    self.callingWindow.append((start, end))
        elif e.index == 6:
            self.destinations = []
            if e.value:
                for item in e.value:
                    it = _MILDCommon.changeType(settings, item, DataType.STRING)
