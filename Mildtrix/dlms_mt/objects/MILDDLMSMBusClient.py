...
class MILDDLMSMBusClient(MILDDLMSObject, IMILDDLMSBase):
    """
    Online help:
    http://www.mildtrix.fi/Mildtrix.DLMS.Objects.MILDDLMSMBusClient
    """
    ...
    def __init__(self, ln=None, sn=0):
        ...
        """
        super(MILDDLMSMBusClient, self).__init__(ObjectType.MBUS_CLIENT, ln, sn)
    ...
    def sendData(self, client, data):
        ...
        _MILDCommon.setObjectCount(len(data), bb)
        for it in data:
            bb.setUInt8(DataType.STRUCTURE)
            bb.setUInt8(3)
            bb.setUInt8(DataType.OCTET_STRING)
            _MILDCommon.setObjectCount(it.DataInformation.Length, bb)
            bb.set(it.DataInformation)
            bb.setUInt8(DataType.OCTET_STRING)
            _MILDCommon.setObjectCount(it.ValueInformation.Length, bb)
            bb.set(it.ValueInformation)
            _MILDCommon.setData(None, bb, _MILDCommon.getDLMSDataType(it.Data), it.Data)
    ...
    def setEncryptionKey(self, client, encryptionKey):
        ...
        if not encryptionKey:
            ...
            _MILDCommon.setObjectCount(encryptionKey.Length, bb)
    ...
    def getValue(self, settings, e):
        ...
        if e.index == 1:
            ...
            ret = _MILDCommon.logicalNameToBytes(self.mBusPortReference)
        elif e.index == 3:
            buff = MILDByteBuffer()
            buff.setUInt8(DataType.ARRAY)
            _MILDCommon.setObjectCount(len(self.captureDefinition), buff)
            for k, v in self.captureDefinition:
                buff.setUInt8(DataType.STRUCTURE)
                buff.setUInt8(2)
                _MILDCommon.setData(settings, buff, DataType.UINT8, k)
                if not v:
                    _MILDCommon.setData(settings, buff, DataType.OCTET_STRING, None)
                else:
                    _MILDCommon.setData(settings, buff, DataType.OCTET_STRING, v.encode())
            ret = buff
        elif e.index == 4:
    ...
    def setValue(self, settings, e):
        if e.index == 1:
            ...
            self.mBusPortReference = _MILDCommon.toLogicalName(e.value)
        elif e.index == 3:
            self.captureDefinition = []
            if e.value:
                for it in e.value:
                    self.captureDefinition.append((_MILDCommon.changeType(settings, it[0], DataType.OCTET_STRING), _MILDCommon.changeType(settings, it[1], DataType.OCTET_STRING)))
        elif e.index == 4:
