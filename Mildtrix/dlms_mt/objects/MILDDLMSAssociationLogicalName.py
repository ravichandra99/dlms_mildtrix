...
class MILDDLMSAssociationLogicalName(MILDDLMSObject, IMILDDLMSBase):
    """
    Online help:
    http://www.mildtrix.fi/Mildtrix.DLMS.Objects.MILDDLMSAssociationLogicalName
    """
    ...
    def __init__(self, ln="0.0.40.0.0.255"):
        ...
        """
        MILDDLMSObject.__init__(self, ObjectType.ASSOCIATION_LOGICAL_NAME, ln)
        self.objectList = MILDDLMSObjectCollection(self)
        self.applicationContextName = MILDApplicationContextName()
        self.xDLMSContextInfo = MILDxDLMSContextType()
        self.authenticationMechanismName = MILDAuthenticationMechanismName()
        self.userList = list()
        self.version = 2
    ...
    def addUser(self, client, id_, name):
        ...
        #  Add structure size.
        data.setUInt8(2)
        _MILDCommon.setData(None, data, DataType.UINT8, id_)
        _MILDCommon.setData(None, data, DataType.STRING, name)
    ...
    def invoke(self, settings, e):
        ...
        if e.index == 1:
            ...
            if settings.authentication == Authentication.HIGH_GMAC:
                ...
                bb = MILDByteBuffer(int(e.parameters))
                bb.getUInt8()
                ic = bb.getUInt32()
            else:
                readSecret = self.secret
            serverChallenge = MILDSecure.secure(settings, settings.cipher, ic, settings.stoCChallenge, readSecret)
            ...
            if accept:
                if settings.authentication == Authentication.HIGH_GMAC or settings.authentication == Authentication.HIGH_ECDSA:
                    ...
                    readSecret = self.secret
                tmp = MILDSecure.secure(settings, settings.cipher, ic, settings.getCtoSChallenge(), readSecret)
    ...
    def getObjects(self, settings, e):
        ...
        if settings.index == 0:
            ...
            _MILDCommon.setObjectCount(len(self.objectList), data)
        pos = 0
        for it in self.objectList:
            ...
            if not pos <= settings.index:
                data.setUInt8(DataType.STRUCTURE)
                data.setUInt8(4)
                _MILDCommon.setData(settings, data, DataType.UINT16, it.objectType)
                _MILDCommon.setData(settings, data, DataType.UINT8, it.version)
                _MILDCommon.setData(settings, data, DataType.OCTET_STRING, _MILDCommon.logicalNameToBytes(it.logicalName))
    ...
    def __getAccessRights(self, settings, item, server, data):
        ...
        if server is None:
            ...
            while pos != cnt:
                ...
                data.setUInt8(DataType.STRUCTURE)
                data.setUInt8(3)
                _MILDCommon.setData(settings, data, DataType.INT8, pos + 1)
                _MILDCommon.setData(settings, data, DataType.ENUM, m)
            ...
            while pos != cnt:
                ...
                if self.version == 0:
                    _MILDCommon.setData(settings, data, DataType.BOOLEAN, m != 0)
                else:
                    _MILDCommon.setData(settings, data, DataType.ENUM, m)
                pos += 1
    ...
    def getUserList(self, settings):
        ...
        if settings.index == 0:
            ...
            data.setUInt8(DataType.ARRAY)
            _MILDCommon.setObjectCount(len(self.userList), data)
        pos = 0
        for k, v in self.userList:
            pos += 1
            if not pos <= settings.index:
                ...
                data.setUInt8(DataType.STRUCTURE)
                data.setUInt8(2)
                _MILDCommon.setData(settings, data, DataType.UINT8, k)
                _MILDCommon.setData(settings, data, DataType.STRING, v)
        return data
    ...
    def getValue(self, settings, e):
        ...
        if e.index == 1:
            ...
            data.setUInt8(DataType.STRUCTURE)
            data.setUInt8(0x7)
            _MILDCommon.setData(settings, data, DataType.UINT8, self.applicationContextName.jointIsoCtt)
            _MILDCommon.setData(settings, data, DataType.UINT8, self.applicationContextName.country)
            _MILDCommon.setData(settings, data, DataType.UINT16, self.applicationContextName.countryName)
            _MILDCommon.setData(settings, data, DataType.UINT8, self.applicationContextName.identifiedOrganization)
            _MILDCommon.setData(settings, data, DataType.UINT8, self.applicationContextName.dlmsUA)
            _MILDCommon.setData(settings, data, DataType.UINT8, self.applicationContextName.applicationContext)
            _MILDCommon.setData(settings, data, DataType.UINT8, self.applicationContextName.contextId)
            ret = data
        elif e.index == 5:
            data = MILDByteBuffer()
            data.setUInt8(DataType.STRUCTURE)
            data.setUInt8(6)
            _MILDCommon.setData(settings, data, DataType.BITSTRING, MILDBitString.toBitString(self.xDLMSContextInfo.conformance, 24))
            _MILDCommon.setData(settings, data, DataType.UINT16, self.xDLMSContextInfo.maxReceivePduSize)
            _MILDCommon.setData(settings, data, DataType.UINT16, self.xDLMSContextInfo.maxSendPduSize)
            _MILDCommon.setData(settings, data, DataType.UINT8, self.xDLMSContextInfo.dlmsVersionNumber)
            _MILDCommon.setData(settings, data, DataType.INT8, self.xDLMSContextInfo.qualityOfService)
            _MILDCommon.setData(settings, data, DataType.OCTET_STRING, self.xDLMSContextInfo.cypheringInfo)
            ret = data
        elif e.index == 6:
            data = MILDByteBuffer()
            data.setUInt8(DataType.STRUCTURE)
            data.setUInt8(0x7)
            _MILDCommon.setData(settings, data, DataType.UINT8, self.authenticationMechanismName.jointIsoCtt)
            _MILDCommon.setData(settings, data, DataType.UINT8, self.authenticationMechanismName.country)
            _MILDCommon.setData(settings, data, DataType.UINT16, self.authenticationMechanismName.countryName)
            _MILDCommon.setData(settings, data, DataType.UINT8, self.authenticationMechanismName.identifiedOrganization)
            _MILDCommon.setData(settings, data, DataType.UINT8, self.authenticationMechanismName.dlmsUA)
            _MILDCommon.setData(settings, data, DataType.UINT8, self.authenticationMechanismName.authenticationMechanismName)
            _MILDCommon.setData(settings, data, DataType.UINT8, self.authenticationMechanismName.mechanismId)
            ret = data
        elif e.index == 7:
            ret = self.secret
        elif e.index == 8:
            ...
            ret = _MILDCommon.logicalNameToBytes(self.securitySetupReference)
        elif e.index == 10:
            ...
            if self.currentUser is None:
                _MILDCommon.setData(settings, data, DataType.UINT8, 0)
                _MILDCommon.setData(settings, data, DataType.STRING, None)
            else:
                _MILDCommon.setData(settings, data, DataType.UINT8, self.currentUser[0])
                _MILDCommon.setData(settings, data, DataType.STRING, self.currentUser[1])
            ret = data
        else:
            e.error = ErrorCode.READ_WRITE_DENIED
        return ret
    ...
    def setValue(self, settings, e):
        if e.index == 1:
            ...
            self.securitySetupReference = _MILDCommon.toLogicalName(e.value)
        elif e.index == 10:
            self.userList = []
            if e.value:
                for tmp in e.value:
    ...
    def load(self, reader):
        ...
        if reader.isStartElement("XDLMSContextInfo", True):
            ...
            self.xDLMSContextInfo.cypheringInfo = MILDByteBuffer.hexToBytes(reader.readElementContentAsString("CypheringInfo"))
    ...
    def save(self, writer):
        ...
        if self.xDLMSContextInfo:
            ...
            writer.writeElementString("CypheringInfo", MILDByteBuffer.hex(self.xDLMSContextInfo.cypheringInfo))
            writer.writeEndElement()
