...
class MILDDLMSSecuritySetup(MILDDLMSObject, IMILDDLMSBase):
    """
    Online help:
    http://www.mildtrix.fi/Mildtrix.DLMS.Objects.MILDDLMSSecuritySetup
    """
    def __init__(self, ln="0.0.43.0.0.255", sn=0):
        ...
        """
        super(MILDDLMSSecuritySetup, self).__init__(ObjectType.SECURITY_SETUP, ln, sn)
        self.version = 1
    ...
    def globalKeyTransfer(self, client, kek, list_):
        ...
        from ..secure.MILDDLMSSecureClient import MILDDLMSSecureClient
        if not list_:
        ...
        for it in list_:
            ...
            bb.setUInt8(2)
            _MILDCommon.setData(None, bb, DataType.ENUM, it.getKey())
            tmp = MILDDLMSSecureClient.encrypt(kek, it)
            _MILDCommon.setData(None, bb, DataType.OCTET_STRING, tmp)
    ...
    def __keyAgreement(self, client, list_):
        ...
        for it in list_:
            ...
            bb.setUInt8(2)
            _MILDCommon.setData(None, bb, DataType.ENUM, it.getKey())
            _MILDCommon.setData(None, bb, DataType.OCTET_STRING, it)
    ...
    def keyAgreement(self, client, type_):
        ...
        # sign = MILDASymmetric.getEphemeralPublicKeySignature(type_ ,
        # client.ciphering.ephemeralKeyPair.public,
        # client.ciphering.signingKeyPair.private)
        #  bb.set(sign)
        # print("Data: " + MILDByteBuffer.hex(data))
        #print("Sign: " + MILDByteBuffer.hex(sign))
        list_ = list()
    ...
    def exportCertificateByEntity(self, client, entity, type_, systemTitle):
        ...
        bb.setUInt8(DataType.ENUM)
        bb.setUInt8(type_)
        _MILDCommon.setData(None, bb, DataType.OCTET_STRING, systemTitle)
    ...
    def exportCertificateBySerial(self, client, serialNumber, issuer):
        ...
        bb.setUInt8(DataType.STRUCTURE)
        bb.setUInt8(2)
        _MILDCommon.setData(None, bb, DataType.OCTET_STRING, serialNumber.encode())
        _MILDCommon.setData(None, bb, DataType.OCTET_STRING, issuer.encode())
        return client.method(self, 7, bb.array(), DataType.STRUCTURE)
        ...
        if e.index == 1:
            ...
            for tmp in e.parameters:
                ...
                if type_ == GlobalKeyType.UNICAST_ENCRYPTION:
                    MILDDLMSSecureClient.decrypt(settings.kek, item[1])
    ...
    def applyKeys(cls, settings, e):
        ...
        for tmp in e.parameters:
            item = tmp
            type_ = GlobalKeyType(item[0])
            data = MILDDLMSSecureClient.decrypt(settings.kek, item[1])
            if type_ == GlobalKeyType.UNICAST_ENCRYPTION:
    ...
    def getCertificatesByteArray(cls, settings):
        ...
        if settings.cipher.certificates:
            _MILDCommon.setObjectCount(len(settings.cipher.certificates), bb)
            for it in settings.cipher.certificates:
                bb.setUInt8(DataType.STRUCTURE)
                _MILDCommon.setObjectCount(6, bb)
                bb.setUInt8(DataType.ENUM)
                bb.setUInt8(CertificateEntity.SERVER)
                bb.setUInt8(DataType.ENUM)
                bb.setUInt8(CertificateType.DIGITAL_SIGNATURE)
                _MILDCommon.addString(it.serialNumber, bb)
                _MILDCommon.addString(it.issuer, bb)
                _MILDCommon.addString(it.subject, bb)
                _MILDCommon.addString("", bb)
        else:
            bb.setUInt8(0)
        return bb.array()
    ...
    def getEphemeralPublicKeyData(cls, keyId, ephemeralKey):
        ...
        #tmp =
        #(MILDAsn1Converter.fromByteArray(ephemeralKey.getEncoded())).get(1)
        #epk = MILDByteBuffer(tmp.value)
    ...
    def load(self, reader):
        ...
        if str_ is None:
            ...
            self.clientSystemTitle = MILDByteBuffer.hexToBytes(str_)
        str_ = reader.readElementContentAsString("ServerSystemTitle")
        if str_ is None:
            self.serverSystemTitle = None
        else:
            self.serverSystemTitle = MILDByteBuffer.hexToBytes(str_)
        self.certificates = []
        if reader.isStartElement("Certificates", True):
            while reader.isStartElement("Item", True):
                it = MILDDLMSCertificateInfo()
    ...
    def save(self, writer):
        ...
        writer.writeElementString("ClientSystemTitle", MILDByteBuffer.hex(self.clientSystemTitle))
        writer.writeElementString("ServerSystemTitle", MILDByteBuffer.hex(self.serverSystemTitle))
