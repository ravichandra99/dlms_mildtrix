...
class MILDDLMSSchedule(MILDDLMSObject, IMILDDLMSBase):
    """
    Online help:
    http://www.mildtrix.fi/Mildtrix.DLMS.Objects.MILDDLMSSchedule
    """
    def __init__(self, ln=None, sn=0):
        ...
        """
        super(MILDDLMSSchedule, self).__init__(ObjectType.SCHEDULE, ln, sn)
    ...
    def delete(self, client, entry):
        ...
        data.setUInt8(2)
        #firstIndex
        _MILDCommon.setData(None, data, DataType.UINT16, entry.index)
        #lastIndex
        _MILDCommon.setData(None, data, DataType.UINT16, entry.index)
    ...
    def enable(self, client, entry):
        ...
        #lastIndex
        _MILDCommon.setData(None, data, DataType.UINT16, entry.index)
        _MILDCommon.setData(None, data, DataType.UINT16, 0)
        _MILDCommon.setData(None, data, DataType.UINT16, 0)
    ...
    def addEntry(cls, it, data):
        ...
        data.setUInt8(6)
        data.set(_MILDCommon.logicalNameToBytes(it.logicalName))
        #Add script selector.
        data.setUInt8(DataType.UINT16)
        data.setUInt16(it.scriptSelector)
        #Add switch time.
        _MILDCommon.setData(None, data, DataType.OCTET_STRING, MILDTime(it.switchTime))
        #Add validity window.
        data.setUInt8(DataType.UINT16)
        data.setUInt16(it.validityWindow)
        #Add exec week days.
        _MILDCommon.setData(None, data, DataType.BITSTRING, MILDBitString.toBitString(it.execWeekdays, 7))
        #Add exec spec days.
        _MILDCommon.setData(None, data, DataType.BITSTRING, it.execSpecDays)
        #Add begin date.
        _MILDCommon.setData(None, data, DataType.OCTET_STRING, MILDDate(it.beginDate))
        #Add end date.
        _MILDCommon.setData(None, data, DataType.OCTET_STRING, MILDDate(it.endDate))
    ...
    def getValue(self, settings, e):
        ...
        if e.index == 2:
            ...
            _MILDCommon.setObjectCount(len(self.entries), data)
    ...
    def createEntry(cls, settings, it):
        item = MILDDLMSScheduleEntry()
        item.index = it[0]
        item.enable = it[1]
        item.logicalName = _MILDCommon.toLogicalName(it[2])
        item.scriptSelector = it[3]
        item.switchTime = _MILDCommon.changeType(settings, it[4], DataType.TIME)
        item.validityWindow = it[5]
        item.execWeekdays = Weekdays(it[6].toInteger())
        item.execSpecDays = str(it[7])
        item.beginDate = _MILDCommon.changeType(settings, it[8], DataType.DATE)
        item.endDate = _MILDCommon.changeType(settings, it[9], DataType.DATE)
        return item
