...
class MILDDLMSRegisterMonitor(MILDDLMSObject, IMILDDLMSBase):
    """
    Online help:
    http://www.mildtrix.fi/Mildtrix.DLMS.Objects.MILDDLMSRegisterMonitor
    """
    def __init__(self, ln=None, sn=0):
        ...
        """
        super(MILDDLMSRegisterMonitor, self).__init__(ObjectType.REGISTER_MONITOR, ln, sn)
        self.thresholds = list()
        self.monitoredValue = MILDDLMSMonitoredValue()
        self.actions = list()
    ...
    def getDataType(self, index):
        ...
        if index == 2:
            return super(MILDDLMSRegisterMonitor, self).getDataType(index)
        if index == 3:
    ...
    def getValue(self, settings, e):
        ...
        if e.index == 3:
            ...
            bb.setUInt8(3)
            #  ClassID
            _MILDCommon.setData(settings, bb, DataType.UINT16, self.monitoredValue.objectType)
            #  LN.
            _MILDCommon.setData(settings, bb, DataType.OCTET_STRING, _MILDCommon.logicalNameToBytes(self.monitoredValue.logicalName))
            #  Attribute index.
            _MILDCommon.setData(settings, bb, DataType.INT8, self.monitoredValue.attributeIndex)
            return bb
        if e.index == 4:
            bb = MILDByteBuffer()
            bb.setUInt8(DataType.STRUCTURE)
            if not self.actions:
                ...
                for it in self.actions:
                    ...
                    bb.setUInt8(2)
                    #  LN
                    _MILDCommon.setData(settings, bb, DataType.OCTET_STRING, _MILDCommon.logicalNameToBytes(it.actionUp.logicalName))
                    #  ScriptSelector
                    _MILDCommon.setData(settings, bb, DataType.UINT16, it.actionUp.scriptSelector)
                    bb.setUInt8(DataType.STRUCTURE)
                    bb.setUInt8(2)
                    #  LN
                    _MILDCommon.setData(settings, bb, DataType.OCTET_STRING, _MILDCommon.logicalNameToBytes(it.actionDown.logicalName))
                    #  ScriptSelector
                    _MILDCommon.setData(settings, bb, DataType.UINT16, it.actionDown.scriptSelector)
            return bb
    ...
    def setValue(self, settings, e):
        if e.index == 1:
            ...
            self.monitoredValue.logicalName = _MILDCommon.toLogicalName(e.value[1])
            self.monitoredValue.attributeIndex = e.value[2]
        elif e.index == 4:
            self.actions = []
            if e.value:
                for as_ in e.value:
                    set_ = MILDDLMSActionSet()
                    target = as_[0]
                    set_.actionUp.logicalName = _MILDCommon.toLogicalName(target[0])
                    set_.actionUp.scriptSelector = target[1]
                    target = as_[1]
                    set_.actionDown.logicalName = _MILDCommon.toLogicalName(target[0])
