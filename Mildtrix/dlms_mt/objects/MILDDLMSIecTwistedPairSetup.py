...
class MILDDLMSIecTwistedPairSetup(MILDDLMSObject, IMILDDLMSBase):
    """
    Online help:
    http://www.mildtrix.fi/Mildtrix.DLMS.Objects.MILDDLMSIecTwistedPairSetup
    """
    ...
    def __init__(self, ln=None, sn=0):
        ...
        """
        MILDDLMSObject.__init__(self, ObjectType.IEC_HDLC_SETUP, ln, sn)
        # Working mode.
    ...
    def load(self, reader):
        ...
        self.primaryAddresses = MILDByteBuffer.hexToBytes(reader.readElementContentAsString("PrimaryAddresses"))
        self.tabis = MILDByteBuffer.hexToBytes(reader.readElementContentAsString("Tabis"))

    def save(self, writer):
        writer.writeElementString("Mode", self.mode)
        writer.writeElementString("Speed", self.speed)
        writer.writeElementString("LN", MILDByteBuffer.hex(self.primaryAddresses))
        if self.tabis:
            writer.writeElementString("LN", MILDByteBuffer.hex(self.tabis))
