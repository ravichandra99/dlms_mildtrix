...
class MILDDLMSPrimeNbOfdmPlcMacNetworkAdministrationData(MILDDLMSObject, IMILDDLMSBase):
    """
    Online help:
    http://www.mildtrix.fi/Mildtrix.DLMS.Objects.MILDDLMSPrimeNbOfdmPlcMacNetworkAdministrationData
    """
    ...
    def __init__(self, ln="0.0.28.5.0.255", sn=0):
        ...
        """
        MILDDLMSObject.__init__(self, ObjectType.PRIME_NB_OFDM_PLC_MAC_NETWORK_ADMINISTRATION_DATA, ln, sn)
    ...
    def __getMulticastEntries(self, settings):
        ...
        if not self.multicastEntries:
            ...
            _MILDCommon.setObjectCount(len(self.multicastEntries), bb)
            for it in self.multicastEntries:
                bb.setUInt8(DataType.STRUCTURE)
                bb.setUInt8(2)
                _MILDCommon.setData(settings, bb, DataType.INT8, it.id)
                _MILDCommon.setData(settings, bb, DataType.INT16, it.members)
        return bb.array()
    ...
    def __getSwitchTable(self, settings):
        ...
        if not self.switchTable:
            ...
            _MILDCommon.setObjectCount(len(self.switchTable), bb)
            for it in self.switchTable:
                _MILDCommon.setData(settings, bb, DataType.INT16, it)
        return bb.array()
    ...
    def __getDirectTable(self, settings):
        ...
        if not self.directTable:
            ...
            _MILDCommon.setObjectCount(len(self.directTable), bb)
            for it in self.directTable:
                bb.setUInt8(DataType.STRUCTURE)
                bb.setUInt8(7)
                _MILDCommon.setData(settings, bb, DataType.INT16, it.sourceSId)
                _MILDCommon.setData(settings, bb, DataType.INT16, it.sourceLnId)
                _MILDCommon.setData(settings, bb, DataType.INT16, it.sourceLcId)
                _MILDCommon.setData(settings, bb, DataType.INT16, it.destinationSId)
                _MILDCommon.setData(settings, bb, DataType.INT16, it.destinationLnId)
                _MILDCommon.setData(settings, bb, DataType.INT16, it.destinationLcId)
                _MILDCommon.setData(settings, bb, DataType.OCTET_STRING, it.did)
        return bb.array()
    ...
    def __getAvailableSwitches(self, settings):
        ...
        if not self.availableSwitches:
            ...
            _MILDCommon.setObjectCount(len(self.availableSwitches), bb)
            for it in self.availableSwitches:
                bb.setUInt8(DataType.STRUCTURE)
                bb.setUInt8(5)
                _MILDCommon.setData(settings, bb, DataType.OCTET_STRING, it.sna)
                _MILDCommon.setData(settings, bb, DataType.INT16, it.lsId)
                _MILDCommon.setData(settings, bb, DataType.INT8, it.level)
                _MILDCommon.setData(settings, bb, DataType.INT8, it.rxLevel)
                _MILDCommon.setData(settings, bb, DataType.INT8, it.rxSnr)
        return bb.array()
    ...
    def __getCommunications(self, settings):
        ...
        if not self.communications:
            ...
            _MILDCommon.setObjectCount(len(self.communications), bb)
            for it in self.communications:
                bb.setUInt8(DataType.STRUCTURE)
                bb.setUInt8(9)
                _MILDCommon.setData(settings, bb, DataType.OCTET_STRING, it.eui)
                _MILDCommon.setData(settings, bb, DataType.INT8, it.txPower)
                _MILDCommon.setData(settings, bb, DataType.INT8, it.txCoding)
                _MILDCommon.setData(settings, bb, DataType.INT8, it.rxCoding)
                _MILDCommon.setData(settings, bb, DataType.INT8, it.rxLvl)
                _MILDCommon.setData(settings, bb, DataType.INT8, it.snr)
                _MILDCommon.setData(settings, bb, DataType.INT8, it.txPowerModified)
                _MILDCommon.setData(settings, bb, DataType.INT8, it.txCodingModified)
                _MILDCommon.setData(settings, bb, DataType.INT8, it.rxCodingModified)
        return bb.array()
    ...
    def __loadDirectTable(cls, reader):
        ...
        if reader.isStartElement("DirectTable", True):
            while reader.isStartElement("Item", True):
                ...
                it.did = MILDByteBuffer.hexToBytes(reader.readElementContentAsString("Did"))
    ...
    def __loadAvailableSwitches(cls, reader):
        ...
        if reader.isStartElement("AvailableSwitches", True):
            while reader.isStartElement("Item", True):
                it = MILDMacAvailableSwitch()
                list_.append(it)
                it.sna = MILDByteBuffer.hexToBytes(reader.readElementContentAsString("Sna"))
    ...
    def __loadCommunications(cls, reader):
        ...
        if reader.isStartElement("Communications", True):
            while reader.isStartElement("Item", True):
                it = MILDMacPhyCommunication()
                list_.append(it)
                it.eui = MILDByteBuffer.hexToBytes(reader.readElementContentAsString("Eui"))
    ...
    def __saveDirectTable(self, writer):
        ...
        if self.directTable:
            for it in self.directTable:
                ...
                writer.writeElementString("Did", MILDByteBuffer.hex(it.did, False))
                writer.writeEndElement()
        writer.writeEndElement()
    ...
    def __saveAvailableSwitches(self, writer):
        ...
        if self.availableSwitches:
            for it in self.availableSwitches:
                writer.writeStartElement("Item")
                writer.writeElementString("Sna", MILDByteBuffer.hex(it.sna, False))
    ...
    def __saveCommunications(self, writer):
        ...
        if self.communications:
            for it in self.communications:
                writer.writeStartElement("Item")
                writer.writeElementString("Eui", MILDByteBuffer.hex(it.eui, False))
