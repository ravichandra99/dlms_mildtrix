...
class MILDDLMSImageTransfer(MILDDLMSObject, IMILDDLMSBase):
    """
    Online help:
    http://www.mildtrix.fi/Mildtrix.DLMS.Objects.MILDDLMSImageTransfer
    """
    ...
    def __init__(self, ln="0.0.44.0.0.255", sn=0):
        ...
        """
        MILDDLMSObject.__init__(self, ObjectType.IMAGE_TRANSFER, ln, sn)
        ...
        if e.index == 1:
            ...
            for it in self.imageActivateInfo:
                if it.identification == imageIdentifier:
                    ...
                    break
            if item is None:
                item = MILDDLMSImageActivateInfo()
                list_.append(item)
    ...
    def getValue(self, settings, e):
        ...
        if e.index == 1:
            ...
            for it in self.imageActivateInfo:
                ...
                #  Item count.
                data.setUInt8(int(3))
                _MILDCommon.setData(settings, data, DataType.UINT32, it.size)
                _MILDCommon.setData(settings, data, DataType.OCTET_STRING, it.identification)
                _MILDCommon.setData(settings, data, DataType.OCTET_STRING, it.signature)
            ret = data
        else:
            e.error = ErrorCode.READ_WRITE_DENIED
        return ret
    ...
    def imageTransferInitiate(self, client, imageIdentifier, forImageSize):
        ...
        data.setUInt8(DataType.STRUCTURE)
        data.setUInt8(2)
        _MILDCommon.setData(None, data, DataType.OCTET_STRING, _MILDCommon.getBytes(imageIdentifier))
        _MILDCommon.setData(None, data, DataType.UINT32, forImageSize)
    ...
    def imageBlockTransfer(self, client, imageBlockValue, imageBlockCount):
        ...
        while pos != cnt:
            ...
            data.setUInt8(DataType.STRUCTURE)
            data.setUInt8(2)
            _MILDCommon.setData(None, data, DataType.UINT32, pos)
            tmp = None
            ...
            _MILDCommon.setData(None, data, DataType.OCTET_STRING, tmp)
    ...
    def load(self, reader):
        ...
        if reader.isStartElement("ImageActivateInfo", True):
            while reader.isStartElement("Item", True):
                it = MILDDLMSImageActivateInfo()
                it.size = reader.readElementContentAsULong("Size")
                it.identification = MILDByteBuffer.hexToBytes(reader.readElementContentAsString("Identification"))
                it.signature = MILDByteBuffer.hexToBytes(reader.readElementContentAsString("Signature"))
    ...
    def save(self, writer):
        ...
        if self.imageActivateInfo:
            for it in self.imageActivateInfo:
                ...
                writer.writeElementString("Identification", MILDByteBuffer.hex(it.identification, False))
                writer.writeElementString("Signature", MILDByteBuffer.hex(it.signature, False))
