...
class MILDDLMSGSMDiagnostic(MILDDLMSObject, IMILDDLMSBase):
    """
    Online help:
    http://www.mildtrix.fi/Mildtrix.DLMS.Objects.MILDDLMSGSMDiagnostic
    """
    ...
    def __init__(self, ln="0.0.25.6.0.255", sn=0):
        ...
        """
        MILDDLMSObject.__init__(self, ObjectType.GSM_DIAGNOSTIC, ln, sn)
        self.version = 1
        self.cellInfo = MILDDLMSGSMCellInfo()
    ...
    def getValue(self, settings, e):
        ...
        if e.index == 1:
            ...
            if self.version == 0:
                bb.setUInt8(4)
                _MILDCommon.setData(settings, bb, DataType.UINT16, self.cellInfo.cellId)
            else:
                bb.setUInt8(7)
                _MILDCommon.setData(settings, bb, DataType.UINT32, self.cellInfo.cellId)
            _MILDCommon.setData(settings, bb, DataType.UINT16, self.cellInfo.locationId)
            _MILDCommon.setData(settings, bb, DataType.UINT8, self.cellInfo.signalQuality)
            _MILDCommon.setData(settings, bb, DataType.UINT8, self.cellInfo.ber)
            if self.version > 0:
                _MILDCommon.setData(settings, bb, DataType.UINT16, self.cellInfo.mobileCountryCode)
                _MILDCommon.setData(settings, bb, DataType.UINT16, self.cellInfo.mobileNetworkCode)
                _MILDCommon.setData(settings, bb, DataType.UINT32, self.cellInfo.channelNumber)
            ret = bb
        elif e.index == 7:
            bb = MILDByteBuffer()
            ...
            for it in self.adjacentCells:
                ...
                bb.setUInt8(2)
                if self.version == 0:
                    _MILDCommon.setData(settings, bb, DataType.UINT16, it.cellId)
                else:
                    _MILDCommon.setData(settings, bb, DataType.UINT32, it.cellId)
                _MILDCommon.setData(settings, bb, DataType.UINT8, it.signalQuality)
            ret = bb
        elif e.index == 8:
