...
class MILDDLMSPushSetup(MILDDLMSObject, IMILDDLMSBase):
    """
    Online help:
    http://www.mildtrix.fi/Mildtrix.DLMS.Objects.MILDDLMSPushSetup
    """
    ...
    def __init__(self, ln="0.7.25.9.0.255", sn=0):
        ...
        """
        super(MILDDLMSPushSetup, self).__init__(ObjectType.PUSH_SETUP, ln, sn)
    ...
    def getValue(self, settings, e):
        ...
        if e.index == 1:
            ...
            _MILDCommon.setObjectCount(len(self.pushObjectList), buff)
            for k, v in self.pushObjectList:
                buff.setUInt8(DataType.STRUCTURE)
                buff.setUInt8(4)
                _MILDCommon.setData(settings, buff, DataType.UINT16, k.objectType)
                _MILDCommon.setData(settings, buff, DataType.OCTET_STRING, _MILDCommon.logicalNameToBytes(k.logicalName))
                _MILDCommon.setData(settings, buff, DataType.INT8, v.attributeIndex)
                _MILDCommon.setData(settings, buff, DataType.UINT16, v.dataIndex)
            ret = buff
        elif e.index == 3:
            buff.setUInt8(DataType.STRUCTURE)
            buff.setUInt8(3)
            _MILDCommon.setData(settings, buff, DataType.ENUM, self.service)
            if self.destination:
                _MILDCommon.setData(settings, buff, DataType.OCTET_STRING, self.destination.encode())
            else:
                _MILDCommon.setData(settings, buff, DataType.OCTET_STRING, None)
            _MILDCommon.setData(settings, buff, DataType.ENUM, self.message)
            ret = buff
        elif e.index == 4:
            buff.setUInt8(DataType.ARRAY)
            _MILDCommon.setObjectCount(len(self.communicationWindow), buff)
            for k, v in self.communicationWindow:
                buff.setUInt8(DataType.STRUCTURE)
                buff.setUInt8(2)
                _MILDCommon.setData(settings, buff, DataType.OCTET_STRING, k)
                _MILDCommon.setData(settings, buff, DataType.OCTET_STRING, v)
            return buff
        elif e.index == 5:
            ...
            if e.value:
                ...
                try:
                    if self.service == ServiceType.HDLC:
                        self.destination = _MILDCommon.toLogicalName(e.value[1])
                    else:
                        self.destination = e.value[1].decode()
                except Exception:
                    self.destination = MILDByteBuffer.toHex(e.value[1])
                self.message = e.value[2]
        elif e.index == 4:
            self.communicationWindow = []
            if e.value:
                for it in e.value:
                    start = _MILDCommon.changeType(settings, it[0], DataType.DATETIME)
                    end = _MILDCommon.changeType(settings, it[1], DataType.DATETIME)
