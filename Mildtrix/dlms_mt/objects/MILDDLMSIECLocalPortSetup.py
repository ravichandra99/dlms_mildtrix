...
class MILDDLMSIECLocalPortSetup(MILDDLMSObject, IMILDDLMSBase):
    """
    Online help:
    http://www.mildtrix.fi/Mildtrix.DLMS.Objects.MILDDLMSIECLocalPortSetup
    """
    ...
    def __init__(self, ln="0.0.20.0.0.255", sn=0):
        ...
        """
        MILDDLMSObject.__init__(self, ObjectType.IEC_LOCAL_PORT_SETUP, ln, sn)
    ...
    def getValue(self, settings, e):
        if e.index == 1:
            ...
            ret = _MILDCommon.getBytes(self.deviceAddress)
        elif e.index == 7:
            ret = _MILDCommon.getBytes(self.password1)
        elif e.index == 8:
            ret = _MILDCommon.getBytes(self.password2)
        elif e.index == 9:
            ret = _MILDCommon.getBytes(self.password5)
        else:
    ...
    def setValue(self, settings, e):
        if e.index == 1:
            ...
            self.deviceAddress = _MILDCommon.changeType(settings, e.value, DataType.STRING)
        elif e.index == 7:
            self.password1 = _MILDCommon.changeType(settings, e.value, DataType.STRING)
        elif e.index == 8:
            self.password2 = _MILDCommon.changeType(settings, e.value, DataType.STRING)
        elif e.index == 9:
            self.password5 = _MILDCommon.changeType(settings, e.value, DataType.STRING)
        else:
            e.error = ErrorCode.READ_WRITE_DENIED
...
class MILDDLMSIECOpticalPortSetup(MILDDLMSIECLocalPortSetup):
    """Obsolete. Use MILDDLMSIECLocalPortSetup instead."""
