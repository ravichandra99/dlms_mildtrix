...
class MILDCiphering(MILDICipher):
    """
    Mildtrix DLMS/COSEM Transport security (Ciphering) settings.
    """
    ...
    def decrypt(cls, c, p, data):
        ...
        tmp = MILDDLMSChippering.decryptAesGcm(p, data)
    ...
    def encrypt(cls, p, data):
        if p.security != Security.NONE:
            tmp = MILDDLMSChippering.encryptAesGcm(p, data)
            return tmp
        return data
    ...
    def generateGmacPassword(self, challenge):
        ...
        bb = MILDByteBuffer()
        MILDDLMSChippering.encryptAesGcm(p, challenge)
        bb.setUInt8(0x10)
