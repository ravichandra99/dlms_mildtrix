...
from .objects.MILDDLMSObjectCollection import MILDDLMSObjectCollection
...
class MILDDLMSSettings:
    ...
    def __init__(self, isServer):
        ...
        self.objects = MILDDLMSObjectCollection()
        self.hdlc = MILDHdlcSettings()
        self.gateway = None
        self.proposedConformance = MILDDLMSSettings.getInitialConformance(self.__useLogicalNameReferencing)
    ...
    def checkFrame(self, frame_, xml):
        ...
        if (self.senderFrame & 0x1) == 0:
            expected = MILDDLMSSettings.increaseReceiverSequence(MILDDLMSSettings.increaseSendSequence(self.receiverFrame))
            if frame_ == expected:
