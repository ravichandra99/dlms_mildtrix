...
class MILDDLMSTranslator:
    ...
    def findNextFrame(self, msg, pdu):
        if not isinstance(msg, (MILDDLMSTranslatorMessage)):
            if not isinstance(msg, (MILDByteBuffer)):
                data = MILDByteBuffer(msg)
            else:
                data = msg
            msg = MILDDLMSTranslatorMessage()
            msg.message = data
        msg.sourceAddress = msg.targetAddress = 0
        original = msg.message.position
        msg.exception = None
        data = msg.message
        settings = MILDDLMSSettings(True)
        reply = MILDReplyData()
        reply.moreData = msg.moreData
        reply.xml = (MILDDLMSTranslatorStructure(self.outputType, self.omitXmlNameSpace, self.hex, self.showStringAsHex, self.comments, self.tags))
        pos = 0
        found = False
        while data.position < len(data):
            if msg.interfaceType in (None, InterfaceType.HDLC, InterfaceType.HDLC_WITH_MODE_E) and data.getUInt8(data.position) == 0x7e:
                ...
                found = MILDDLMS.getData(settings, data, reply, None)
                data.position = pos
                if found:
                    break
                ...
                if found:
                    break
            elif msg.interfaceType in (None, InterfaceType.WIRED_MBUS) and MILDDLMS.isWiredMBusData(msg.message):
                pos = data.position
                ...
                if found:
                    break
            elif msg.interfaceType in (None, InterfaceType.WIRELESS_MBUS) and MILDDLMS.isWirelessMBusData(msg.message):
                pos = data.position
    ...
    def getPdu(self, value):
        return self.getPdu(MILDByteBuffer(value))
    ...
    def getDlmsFraming(cls, value):
        ...
        while pos != len(value):
            ...
            if MILDDLMS.isWirelessMBusData(value):
    ...
    def getPdu_0(self, value):
        ...
        data = MILDReplyData()
        data.xml = (MILDDLMSTranslatorStructure(self.outputType, self.omitXmlNameSpace, self.hex, self.showStringAsHex, self.comments, self.tags))
        settings = MILDDLMSSettings(True)
        settings.interfaceType = framing
        MILDDLMS.getData(settings, value, data, None)
        return data.data

    def getCiphering(self, settings, force):
        if force or self.security != Security.NONE:
            c = MILDCiphering(self.systemTitle)
    ...
    def messageToXml(self, msg):
        ...
        msg.exception = None
        xml = MILDDLMSTranslatorStructure(self.outputType, self.omitXmlNameSpace, self.hex, self.showStringAsHex, self.comments, self.tags)
        data = MILDReplyData()
        settings = MILDDLMSSettings(True)
        ...
        try:
            ...
            if msg.interfaceType in (None, InterfaceType.HDLC, InterfaceType.HDLC_WITH_MODE_E) and msg.message.getUInt8(msg.message.position) == 0x7e:
                ...
                if MILDDLMS.getData(settings, msg.message, data, None):
            ...
            if msg.interfaceType in (None, InterfaceType.WRAPPER) and msg.message.getUInt16(msg.message.position) == 1:
                ...
                MILDDLMS.getData(settings, msg.message, data, None)
                msg.moreData = data.moreData
                msg.sourceAddress = data.sourceAddress
                msg.targetAddress = data.targetAddress
                if not self.pduOnly:
            ...
            if msg.interfaceType in (None, InterfaceType.PLC) and msg.message.getUInt8(msg.message.position) == 2:
                ...
                return msg.xml
            #If Wired M-Bus.
            if msg.interfaceType in (None, InterfaceType.WIRED_MBUS) and MILDDLMS.isWiredMBusData(msg.message):
                ...
                return msg.xml
            #If Wireless M-Bus.
            if msg.interfaceType in (None, InterfaceType.WIRELESS_MBUS) and MILDDLMS.isWirelessMBusData(msg.message):
    ...
    #
    # @param pdu
    #            Converted hex string or MILDByteBuffer.
    # Converted XML.
    #
    def pduToXml(self, pdu):
        if not isinstance(pdu, MILDByteBuffer):
            pdu = MILDByteBuffer(pdu)
    ...
    def getUa(cls, data, xml):
        ...
        while data.position < len(data):
            ...
            if len_ == 1:
                ...
                raise MILDDLMSException("Invalid Exception.")
            if id_ == _HDLCInfo.MAX_INFO_TX:
                ...
                raise MILDDLMSException("Invalid UA response.")
    ...
    def __pduToXml_(self, xml, value, omitDeclaration, omitNameSpace, allowUnknownCommand=True):
        ...
        if cmd == Command.AARQ:
            value.position = 0
            _MILDAPDU.parsePDU(settings, settings.cipher, value, xml)
        elif cmd == Command.INITIATE_REQUEST:
            value.position = 0
            settings = MILDDLMSSettings(True)
            _MILDAPDU.parseInitiate(True, settings, settings.cipher, value, xml)
        elif cmd == Command.INITIATE_RESPONSE:
            value.position = 0
            settings = MILDDLMSSettings(False)
            self.getCiphering(settings, True)
            _MILDAPDU.parseInitiate(True, settings, settings.cipher, value, xml)
        elif cmd == 0x81:
            #  Ua
            ...
            MILDDLMSLNCommandHandler.handleGetRequest(settings, None, value, None, xml)
        elif cmd == Command.SET_REQUEST:
            MILDDLMSLNCommandHandler.handleSetRequest(settings, None, value, None, xml)
        elif cmd == Command.READ_REQUEST:
            MILDDLMSSNCommandHandler.handleReadRequest(settings, None, value, None, xml)
        elif cmd == Command.METHOD_REQUEST:
            MILDDLMSLNCommandHandler.handleMethodRequest(settings, None, value, None, None, xml)
        elif cmd == Command.WRITE_REQUEST:
            MILDDLMSSNCommandHandler.handleWriteRequest(settings, None, value, None, xml)
        elif cmd == Command.ACCESS_REQUEST:
            MILDDLMSLNCommandHandler.handleAccessRequest(settings, None, value, None, xml)
            ...
            data.xml = (xml)
            data.data = value
            MILDDLMSSNCommandHandler.handleInformationReport(settings, data, None)
        elif cmd == Command.EVENT_NOTIFICATION:
            data.xml = (xml)
            data.data = value
            MILDDLMSLNCommandHandler.handleEventNotification(settings, data, None)
            ...
            settings.cipher = MILDCiphering("ABCDEFGH".encode())
            data.xml = (xml)
            data.data = value
            value.position = 0
            MILDDLMS.getPdu(settings, data)
            ...
            if value.available() != 0:
                ...
                if value.available() != 0:
                    _MILDAPDU.parsePDU2(settings, settings.cipher, value, xml)
            ...
            if settings.cipher and self.comments:
                ...
                try:
                    ...
                    if st or cmd in (Command.GENERAL_GLO_CIPHERING, Command.GENERAL_DED_CIPHERING):
                        ...
                        if p.blockCipherKey:
                            data2 = MILDByteBuffer(MILDCiphering.decrypt(settings.cipher, p, value))
            ...
            cnt = _MILDCommon.getObjectCount(value)
            ...
            if settings.cipher and self.comments:
                ...
                try:
                    ...
                    p.xml = xml
                    tmp = MILDByteBuffer(MILDCiphering.decrypt(settings.cipher, p, tmp))
                    len_ = xml.getXmlLength()
                    ...
                    xml.setXmlLength(len_)
                value.position = originalPosition
            len_ = _MILDCommon.getObjectCount(value)
            tmp = bytearray(len_)
            value.get(tmp)
            xml.appendStartTag(Command.GENERAL_GLO_CIPHERING)
            xml.appendLine(TranslatorTags.SYSTEM_TITLE, None, MILDByteBuffer.hex(tmp, False, 0, len_))
            len_ = _MILDCommon.getObjectCount(value)
            tmp = bytearray(len_)
            value.get(tmp)
            xml.appendLine(TranslatorTags.CIPHERED_SERVICE, None, MILDByteBuffer.hex(tmp, False, 0, len_))
            ...
            xml.appendLine(TranslatorTags.PHYSICAL_DEVICE_ADDRESS, None, MILDByteBuffer.hex(tmp, False, 0, len_))
            self.__pduToXml_(xml, MILDByteBuffer(value.remaining()), omitDeclaration, omitNameSpace, allowUnknownCommand)
            xml.appendEndTag(cmd)
    ...
    def getCommand(cls, node, s, tag):
        ...
        if tag in (Command.SNRM, Command.AARQ, Command.READ_REQUEST, Command.WRITE_REQUEST,\
           ...
           Command.DED_GET_REQUEST, Command.DED_SET_REQUEST, Command.DED_METHOD_REQUEST):
            s.settings.server = False
            tmp = MILDByteBuffer.hexToBytes(cls.getValue(node, s))
    ...
    def handleAarqAare(cls, node, s, tag):
        ...
        if tag == TranslatorGeneralTags.APPLICATION_CONTEXT_NAME:
            ...
            if s.outputType == TranslatorOutputType.STANDARD_XML:
                ...
                _MILDAPDU.parseInitiate(False, s.settings, s.settings.getCipher(), bb, None)
            ...
            if s.settings.authentication == Authentication.LOW:
                s.settings.password = MILDByteBuffer.hexToBytes(cls.getValue(node, s))
            else:
                s.settings.setCtoSChallenge(MILDByteBuffer.hexToBytes(cls.getValue(node, s)))
            ...
            s.settings.setStoCChallenge(MILDByteBuffer.hexToBytes(cls.getValue(node, s)))
        elif tag == 0xBE03:
            pass
        elif tag == 0xBE05:
            if s.settings.isServer:
            ...
            _MILDCommon.setBitString(pv, str_, False)
    ...
    def readNode(cls, node, s):
        ...
        if s.command == Command.NONE:
            ...
            if not cls.handleAarqAare(node, s, tag):
                return
        elif tag >= _MILDCommon.DATA_TYPE_OFFSET:
            if tag == DataType.DATETIME + _MILDCommon.DATA_TYPE_OFFSET or (s.command == Command.EVENT_NOTIFICATION and not s.attributeDescriptor):
            ...
            if tag in (Command.GET_REQUEST << 8 | GetCommandType.NORMAL,
                ...
                if s.command == Command.WRITE_REQUEST:
                    _MILDCommon.setObjectCount(cls.getNodeCount(node), s.data)
                ...
                if s.outputType == TranslatorOutputType.SIMPLE_XML:
                    ...
                    str_ = cls.getValue(node, s)
                    s.setTime(MILDDateTime(_MILDCommon.getGeneralizedTime(str_)))
                ...
                s.settings.stoCChallenge = MILDByteBuffer.hexToBytes(cls.getValue(node, s))
                ...
                s.attributeDescriptor.set(MILDByteBuffer.hexToBytes(cls.getValue(node, s)))
                ...
                if s.command == Command.GET_RESPONSE:
                    s.data.setUInt8(0)
                tmp = MILDByteBuffer.hexToBytes(cls.getValue(node, s))
                _MILDCommon.setObjectCount(len(tmp), s.data)
                s.data.set(tmp)
                ...
                if s.command == Command.GET_REQUEST or s.requestType == 3:
                    _MILDCommon.setObjectCount(node.getChildNodes().getLength(), s.attributeDescriptor)
                ...
                _MILDCommon.setObjectCount(cls.getNodeCount(node), s.attributeDescriptor)
                ...
                if s.command == Command.GENERAL_CIPHERING:
                    ...
                    _MILDCommon.setObjectCount(4 + len(tmp), s.data)
                    s.data.setUInt8(4)
                    _MILDCommon.setObjectCount(2 + len(tmp), s.data)
                ...
                s.data.set(MILDByteBuffer.hexToBytes(cls.getValue(node, s)))
                ...
                s.setPhysicalDeviceAddress(MILDByteBuffer.hexToBytes(cls.getValue(node, s)))
        ...
        for node2 in node:
            ...
            cnt += 1
        if preData:
            _MILDCommon.setObjectCount(cnt, preData)
            preData.set(s.data)
            s.data.size = 0
            s.data.set(preData)
            ...
            for r in str_.split("\n"):
                if r.strip() and not r == "\r":
                    ...
                    for it in r.strip().split(";"):
                        tmp.clear()
                        _MILDCommon.setData(None, tmp, types[len(types)], MILDDLMSConverter.changeType(it, types[len(types)]))
                        if len(tmp) == 1:
                            s.data.setUInt8(0)
                        else:
                        ...
                        col += 1
            row += 1
        _MILDCommon.setObjectCount(len(tmp2), s.data)
        s.data.set(tmp2)
    ...
    def getNodeTypes(cls, s, node):
        ...
        if len1 > 1:
            ...
            _MILDCommon.setObjectCount(len, s.data)
        for node2 in node:
            if s.outputType == TranslatorOutputType.SIMPLE_XML:
                ...
                str_ = node2.tag
            tag = s.tags.get(str_)
            s.data.setUInt8(tag - _MILDCommon.DATA_TYPE_OFFSET)

    @classmethod
    def updateDateTime(cls, node, s, preData):
        bb = preData
        if s.requestType != 0xFF:
            bb = cls.updateDataType(node, s, DataType.DATETIME + _MILDCommon.DATA_TYPE_OFFSET)
        else:
            ...
            if tmp:
                if len(tmp) == 5:
                    ...
                    dt = DataType.TIME
                s.setTime(_MILDCommon.changeType(None, tmp, dt))
        return bb
    ...
    def updateDataType(cls, node, s, tag):
        ...
        if s.template or v == "*":
            s.template = True
            return preData
        dt = DataType(tag - _MILDCommon.DATA_TYPE_OFFSET)
        if dt == DataType.ARRAY:
            s.data.setUInt8(DataType.ARRAY)
            preData = MILDByteBuffer(s.data)
            s.data.size = 0
        elif dt == DataType.BCD:
            _MILDCommon.setData(None, s.data, DataType.BCD, s.parseShort(cls.getValue(node, s)))
        elif dt == DataType.BITSTRING:
            _MILDCommon.setData(None, s.data, DataType.BITSTRING, cls.getValue(node, s))
        elif dt == DataType.BOOLEAN:
            _MILDCommon.setData(None, s.data, DataType.BOOLEAN, s.parseShort(cls.getValue(node, s)))
        elif dt == DataType.DATE:
            _MILDCommon.setData(None, s.data, DataType.DATE, _MILDCommon.changeType(None, MILDByteBuffer.hexToBytes(cls.getValue(node, s)), DataType.DATE))
            ...
            if len(tmp) == 5:
                ...
                dt = DataType.TIME
            _MILDCommon.setData(None, s.data, dt, _MILDCommon.changeType(None, tmp, dt))
        elif dt == DataType.ENUM:
            _MILDCommon.setData(None, s.data, DataType.ENUM, s.parseShort(cls.getValue(node, s)))
        elif dt == DataType.FLOAT32:
            cls.getFloat32(node, s)
        elif dt == DataType.FLOAT64:
            cls.getFloat64(node, s)
        elif dt == DataType.INT16:
            _MILDCommon.setData(None, s.data, DataType.INT16, s.parseShort(cls.getValue(node, s)))
        elif dt == DataType.INT32:
            _MILDCommon.setData(None, s.data, DataType.INT32, s.parseInt(cls.getValue(node, s)))
        elif dt == DataType.INT64:
            _MILDCommon.setData(None, s.data, DataType.INT64, s.parseLong(cls.getValue(node, s)))
        elif dt == DataType.INT8:
            _MILDCommon.setData(None, s.data, DataType.INT8, s.parseShort(cls.getValue(node, s)))
        elif dt == DataType.NONE:
            _MILDCommon.setData(None, s.data, DataType.NONE, None)
        elif dt == DataType.OCTET_STRING:
            cls.getOctetString(node, s)
        elif dt == DataType.STRING:
            if s.showStringAsHex:
                _MILDCommon.setData(None, s.data, DataType.STRING, MILDByteBuffer.hexToBytes(cls.getValue(node, s)))
            else:
                _MILDCommon.setData(None, s.data, DataType.STRING, cls.getValue(node, s))
        elif dt == DataType.STRING_UTF8:
            if s.showStringAsHex:
                _MILDCommon.setData(None, s.data, DataType.STRING_UTF8, MILDByteBuffer.hexToBytes(cls.getValue(node, s)))
            else:
                _MILDCommon.setData(None, s.data, DataType.STRING_UTF8, cls.getValue(node, s))
        elif dt == DataType.STRUCTURE:
            s.data.setUInt8(DataType.STRUCTURE)
            preData = MILDByteBuffer(s.data)
            s.data.size = 0
        elif dt == DataType.TIME:
            _MILDCommon.setData(None, s.data, DataType.TIME, _MILDCommon.changeType(None, MILDByteBuffer.hexToBytes(cls.getValue(node, s)), DataType.TIME))
        elif dt == DataType.UINT16:
            _MILDCommon.setData(None, s.data, DataType.UINT16, s.parseShort(cls.getValue(node, s)))
        elif dt == DataType.UINT32:
            _MILDCommon.setData(None, s.data, DataType.UINT32, s.parseLong(cls.getValue(node, s)))
        elif dt == DataType.UINT64:
            _MILDCommon.setData(None, s.data, DataType.UINT64, s.parseLong(cls.getValue(node, s)))
        elif dt == DataType.UINT8:
            _MILDCommon.setData(None, s.data, DataType.UINT8, s.parseShort(cls.getValue(node, s)))
    ...
    def getOctetString(cls, node, s):
        ...
        _MILDCommon.setData(None, s.data, DataType.OCTET_STRING, bb.array())

    @classmethod
    def getFloat32(cls, node, s):
        bb = MILDByteBuffer()
        bb.setHexString(cls.getValue(node, s))
        _MILDCommon.setData(None, s.data, DataType.FLOAT32, bb.getFloat())

    @classmethod
    def getFloat64(cls, node, s):
        bb = MILDByteBuffer()
        bb.setHexString(cls.getValue(node, s))
        _MILDCommon.setData(None, s.data, DataType.FLOAT64, bb.getDouble())

    def xmlToHexPdu(self, xml, addSpace=False):
        return MILDByteBuffer.hex(self.xmlToPdu(xml, None), addSpace)

    def xmlToPdu(self, xml, settings=None):
        s = settings
        if s is None:
            s = MILDDLMSXmlSettings(self.outputType, self.hex, self.showStringAsHex, self.tagsByName)
        ...
        if s.command == Command.INITIATE_REQUEST:
            _MILDAPDU.getInitiateRequest(s.settings, bb)
        elif s.command == Command.INITIATE_RESPONSE:
            bb.set(_MILDAPDU.getUserInformation(s.settings, s.settings.cipher))
        elif s.command in (Command.READ_REQUEST, Command.WRITE_REQUEST, Command.READ_RESPONSE, Command.WRITE_RESPONSE):
            sn = MILDDLMSSNParameters(s.settings, s.command, s.count, s.requestType, s.attributeDescriptor, s.data)
            MILDDLMS.getSNPdu(sn, bb)
            ...
            ln = MILDDLMSLNParameters(s.settings, 0, s.command, s.requestType, s.attributeDescriptor, s.data, 0xff)
            MILDDLMS.getLNPdu(ln, bb)
            ...
            _MILDCommon.setObjectCount(len(s.data), bb)
            bb.set(s.data)
            ...
            if s.data:
                ...
                s.data.set(bb)
                bb.clear()
            bb.set(MILDDLMS.getHdlcFrame(s.settings, int(Command.SNRM), s.data))
            ...
            if s.data:
                ...
                s.data.set(bb)
                bb.clear()
            bb.set(MILDDLMS.getHdlcFrame(s.settings, s.command, s.data))
        elif s.command in (Command.AARQ, Command.GLO_INITIATE_REQUEST):
            _MILDAPDU.generateAarq(s.settings, s.settings.cipher, s.data, bb)
        elif s.command in (Command.AARE, Command.GLO_INITIATE_RESPONSE):
            _MILDAPDU.generateAARE(s.settings, bb, s.result, s.diagnostic, s.settings.cipher, s.attributeDescriptor, s.data)
            ...
            ln = MILDDLMSLNParameters(s.settings, 0, s.command, s.requestType, s.attributeDescriptor, s.data, 0xff, Command.NONE)
            MILDDLMS.getLNPdu(ln, bb)
            ...
            bb.setUInt8(s.command)
            _MILDCommon.setObjectCount(len(s.settings.sourceSystemTitle), bb)
        ...
        if s.physicalDeviceAddress:
            ...
            _MILDCommon.setObjectCount(len(s.physicalDeviceAddress), bb2)
    ...
    def dataToXml(self, data):
        ...
        xml = MILDDLMSTranslatorStructure(self.outputType, self.omitXmlNameSpace, self.hex, self.showStringAsHex, self.comments, self.tags)
        di.xml = xml
        _MILDCommon.getData(None, data, di)
        return str(di.xml)
    ...
    def __getAllDataNodes(self, nodes, s):
        ...
        for it in nodes:
            ...
            if tag == TranslatorTags.RAW_DATA:
                ...
                if preData:
                    _MILDCommon.setObjectCount(it.getChildNodes().getLength(), preData)
                    preData.set(s.data)
                    s.data.size(0)
                    s.data.set(preData)
